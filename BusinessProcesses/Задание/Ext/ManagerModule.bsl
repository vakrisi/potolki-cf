...
Процедура ПриПеренаправленииЗадачи(ЗадачаСсылка, НоваяЗадачаСсылка) Экспорт
	...
	БизнесПроцессОбъект.РезультатИсполнения = РезультатВыполненияПриПеренаправлении(ЗадачаСсылка)
		+ БизнесПроцессОбъект.РезультатИсполнения;
	//--
...
Процедура УстановитьСостояниеЭлементовФормыЗадачи(Форма) Экспорт
	...
	Форма.Элементы.ПредметЗадачи.Гиперссылка = Форма.Объект.ПредметЗадачи <> Неопределено И НЕ Форма.Объект.ПредметЗадачи.Пустая();
	Форма.ПредметСтрокой = ОбщегоНазначения.ПредметСтрокой(Форма.Объект.ПредметЗадачи);
	//--
...
Функция РезультатВыполненияПриПеренаправлении(Знач ЗадачаСсылка)
	...
	Комментарий = СокрЛП(ЗадачаСсылка.РезультатИсполнения);
	//--
...
// Добавляет команду создания Задачи
//
// Параметры:
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.БизнесПроцессы.Задание) Тогда
		...
		КомандаСоздатьНаОсновании.Представление = Метаданные.БизнесПроцессы.Задание.Представление();
		// #13969 добавлено
