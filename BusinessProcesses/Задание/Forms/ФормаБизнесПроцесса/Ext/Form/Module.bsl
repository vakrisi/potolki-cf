...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Карта = БизнесПроцессы.Задание.ПолучитьКартуМаршрута();
		ЭтоНовый = Истина;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ИзменятьЗаданияЗаднимЧислом = Истина; //ПолучитьФункциональнуюОпцию("ИзменятьЗаданияЗаднимЧислом");
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если НЕ ЗавершениеРаботы И ЭтоНовый Тогда	
		НомерЛида = ПолучитьНомерЛидаОснования();
		Если НЕ Объект.Ссылка.Пустая() Тогда 
			Оповестить("ЗадачаПоЛидуСоздана", НомерЛида);
		КонецЕсли;
		ЭтоНовый = Ложь;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.НеобходимаПроверка");
...
Процедура ИнициализацияФормы()
	...
	ИзменятьЗаданияЗаднимЧислом           = Истина; //ПолучитьФункциональнуюОпцию("ИзменятьЗаданияЗаднимЧислом");
	ИспользоватьПодчиненныеБизнесПроцессы = Ложь; //ПолучитьФункциональнуюОпцию("ИспользоватьПодчиненныеБизнесПроцессы");
	
	ПредметСтрокой = ОбщегоНазначения.ПредметСтрокой(Объект.ПредметЗадачи);
...
Процедура УстановитьДоступностьПроверяющего(Форма)
	...
	ДоступностьПоля = Форма.Объект.НеобходимаПроверка;
...
Функция ПолучитьНомерЛидаОснования()
	...
	Если ТипЗнч(Объект.ПредметЗадачи) = Тип("ДокументСсылка.Лид") Тогда
		Возврат Объект.ПредметЗадачи.Номер;
	Иначе
		Возврат "";
	КонецЕсли;
