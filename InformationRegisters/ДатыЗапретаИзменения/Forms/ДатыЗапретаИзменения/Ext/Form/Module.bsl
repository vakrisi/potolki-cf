...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ #12103 удаляем элемент "НетЗапрета"
	//Элементы.УстановкаДатыЗапрета.СписокВыбора.Удалить(0);
	//--
	...
	//#9910
	ОбщийМодульСервер.ДополнитьОписаниеДатыЗапрета(Элементы.ОписаниеДатыЗапрета);
	...
	//++ Потолки Модификация
	ЗапретИзмененияДанныхПрошлыхПериодов = Константы.ИспользоватьДатыЗапретаИзменения.Получить();	
	ОбновитьВидимостьПоЗапретИзмененияДанныхПрошлыхПериодов();
	//-- Потолки Модификация
	...
	Элементы.ГруппаБольшеВозможностей.Видимость = Ложь;
	Элементы.МеньшеВозможностей.Видимость = Ложь;
	//-- #12103
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("РегистрСведений.ДатыЗапретаИзменения.Форма.РедактированиеДатыЗапрета") Тогда
		...
		Если ВыбранноеЗначение <> Неопределено Тогда
			...
			Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
				...
				//#9910
				Строка.СтаршеУказанныхДней         		= ВыбранноеЗначение.СтаршеУказанныхДней;
				Строка.СтаршеУказанныхМесяцев         	= ВыбранноеЗначение.СтаршеУказанныхМесяцев;
...
Процедура ОписаниеДатыЗапретаПриИзменении(Элемент)
	...
	//++ #16658 чтобы не сбрасывалось на "БезЗапрета"
	Если ОписаниеДатыЗапрета = "ПроизвольнаяДата" Тогда
		ДатаЗапрета = ТекущаяДата();	
	КонецЕсли;
	//-- #16658
...
Процедура ПользователиОбработкаВыбораНаСервере(ВыбранноеЗначение)
	...
	Для Каждого Значение Из ВыбранноеЗначение Цикл
		...
		Если ПользователиДатЗапрета.НайтиСтроки(Отбор).Количество() = 0 Тогда
			//++ Потолки (16982): для новых пользователей д.б. "Не установлена"
...
Процедура УстановитьУсловноеОформление()
	...
	//++ Потолки (#16658) Оформление пустой даты (из старой версии)
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", "Без запрета");
...
Процедура ОбновитьНаСервере()
	...
	Если УстановкаДатыЗапрета = "НетЗапрета" Тогда
		УстановкаДатыЗапрета = "ДляВсехПользователей";
	КонецЕсли;
	//--
	УстановкаДатыЗапретаНовое = УстановкаДатыЗапрета;
...
Функция ДатыЗапретаПользователя(Знач Форма)
	...
	ПоляСтроки = "ПолноеПредставление, Представление, Раздел, Объект,
				 ...
				 //#9910
				 |СтаршеУказанныхДней, СтаршеУказанныхМесяцев";
...
Функция ПрочитатьДанныеПользователяСРазделами(Знач Пользователь,
	...
	//++#9910
	|	0 КАК СтаршеУказанныхДней,
	|	0 КАК СтаршеУказанныхМесяцев,
		//--#9910
...
Функция ЗаблокироватьЗаписьПользователяНаСервере(Знач АдресУстановленныхБлокировок, Знач Раздел, Знач Объект,
	...
	//++ Потолки (#9910)
	ПрочитанныеСвойства.Вставить("СтаршеУказанныхДней");
	ПрочитанныеСвойства.Вставить("СтаршеУказанныхМесяцев");
	//-- Потолки (#9910)
...
Функция ТекущиеДанныеОбщейДатыЗапрета()
	...
	//#9910
	Данные.Вставить("СтаршеУказанныхДней", 		СтаршеУказанныхДней);
	Данные.Вставить("СтаршеУказанныхМесяцев", 	СтаршеУказанныхМесяцев);
...
Функция ТекущийСпособУказанияДатыЗапрета(Знач Пользователь, Знач ЕдинственныйРаздел, Знач ЗначениеДляВсехПользователей, Знач НачалоДня, Данные = Неопределено)
	...
	Если Данные = Неопределено Тогда
		...
		//++ Потолки (#9910)
		Данные.Вставить("СтаршеУказанныхДней", 0);
		Данные.Вставить("СтаршеУказанныхМесяцев", 0);
		//--
	КонецЕсли;
...
Процедура РедактироватьДатуЗапретаВФормеЗавершение(ВыделенныеСтроки) Экспорт
	...
	//++#9910
	ПараметрыФормы.Вставить("СтаршеУказанныхДней", Элементы.ДатыЗапрета.ТекущиеДанные.СтаршеУказанныхДней);
	ПараметрыФормы.Вставить("СтаршеУказанныхМесяцев", Элементы.ДатыЗапрета.ТекущиеДанные.СтаршеУказанныхМесяцев);
	//--#9910
...
Функция ВнутренняяДатаЗапрета(Данные)
	...
	Если ЗначениеЗаполнено(Данные.ОписаниеДатыЗапрета)
	   И Данные.ОписаниеДатыЗапрета <> "ПроизвольнаяДата"
	   //++ Потолки (#16658)
	   И Данные.ОписаниеДатыЗапрета <> "БезЗапрета"
	   //-- Потолки (#16658)
	   Тогда
...
Функция ВнутреннееОписаниеДатыЗапрета(Знач Данные)
	...
	Если Данные.ОписаниеДатыЗапрета <> "ПроизвольнаяДата" Тогда
		...
		//++ Потолки (#9910)
		Если Данные.ОписаниеДатыЗапрета = "СтаршеУказанныхДней"  Тогда
			ВнутреннееОписание = ВнутреннееОписание + Символы.ПС + Формат(Данные.СтаршеУказанныхДней, "ЧГ=0");	
		КонецЕсли;
		Если Данные.ОписаниеДатыЗапрета = "СтаршеУказанныхМесяцев" Тогда
			ВнутреннееОписание = ВнутреннееОписание + Символы.ПС + Формат(Данные.СтаршеУказанныхМесяцев, "ЧГ=0");	
		КонецЕсли;
		//-- Потолки (#9910)
	КонецЕсли;
	
	//++ Потолки (#16658)
	Если Данные.ОписаниеДатыЗапрета = "БезЗапрета" Тогда
	   ВнутреннееОписание = "";
	КонецЕсли;
	//-- Потолки (#16658)
...
Процедура ЗаполнитьПоВнутреннемуОписаниюДатыЗапрета(Знач Данные, Знач ВнутреннееОписание)
	...
	//++#9910
	Данные.СтаршеУказанныхДней = 0;
	Данные.СтаршеУказанныхМесяцев = 0;
	//--#9910
	...
	Если ЗначениеЗаполнено(ВнутреннееОписание) Тогда
		...
		Иначе
			...
			Если ЗначениеЗаполнено(КоличествоДнейРазрешения) Тогда
				...
				//++#9910
				Если ОписаниеДатыЗапрета = "СтаршеУказанныхДней" Тогда
					Данные.СтаршеУказанныхДней = ОписаниеТипаЧисло.ПривестиЗначение(КоличествоДнейРазрешения);
					Данные.КоличествоДнейРазрешения = 0;
				ИначеЕсли ОписаниеДатыЗапрета = "СтаршеУказанныхМесяцев" Тогда
					Данные.СтаршеУказанныхМесяцев = ОписаниеТипаЧисло.ПривестиЗначение(КоличествоДнейРазрешения);
					Данные.КоличествоДнейРазрешения = 0;
				КонецЕсли;
				//--#9910
	...
	//++#16658
	Если (ОписаниеДатыЗапрета = "" ИЛИ ОписаниеДатыЗапрета = "ПроизвольнаяДата")
		И Данные.ДатаЗапрета = '00010101'
		Тогда
		ОписаниеДатыЗапрета = "БезЗапрета";
	КонецЕсли;
	//++#16658
...
&НаКлиенте
Процедура ЗапретИзмененияДанныхПрошлыхПериодовПриИзменении(Элемент)
	ЗапретИзмененияДанныхПрошлыхПериодовПриИзмененииНаСервере();
КонецПроцедуры

&НаСервере
Процедура ЗапретИзмененияДанныхПрошлыхПериодовПриИзмененииНаСервере()
	Константы.ИспользоватьДатыЗапретаИзменения.Установить(ЗапретИзмененияДанныхПрошлыхПериодов);
	ОбновитьВидимостьПоЗапретИзмененияДанныхПрошлыхПериодов();
	Если ЗапретИзмененияДанныхПрошлыхПериодов Тогда
		ПриИзмененииИспользованияДатЗапретаНаСервере();
	КонецЕсли;
КонецПроцедуры

Процедура ОбновитьВидимостьПоЗапретИзмененияДанныхПрошлыхПериодов()
	Элементы.ГруппаШапка.Видимость = ЗапретИзмененияДанныхПрошлыхПериодов;
	Элементы.ИспользованиеДатЗапрета.Видимость = ЗапретИзмененияДанныхПрошлыхПериодов;
	Элементы.НастройкаДатыЗапрета.Видимость = ЗапретИзмененияДанныхПрошлыхПериодов;
КонецПроцедуры
