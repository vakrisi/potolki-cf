...
&НаСервере
Процедура ОбновитьПредставлениеКурса()
	
	КрупныйШрифт = Новый Шрифт(,11);

	ВалютаУчета = ПолучитьФункциональнуюОпцию("ВалютаУчета");
	ДатаКурса = Запись.Период;
	
	КомпонентыФС = Новый Массив;
	КомпонентыФС.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Курс'") + " ", КрупныйШрифт));
	
	СуммаСтрокой = Формат(Запись.Курс, "ЧДЦ=8; ЧРД=,; ЧРГ=' '; ЧН=0,0000");		
	Элементы.ПредставлениеКурсаВалюты.Заголовок = СтрШаблон("%1 %2 = %3 %4", Запись.Кратность, ВалютаСписок.Наименование, СуммаСтрокой, ВалютаУчета.Наименование);
