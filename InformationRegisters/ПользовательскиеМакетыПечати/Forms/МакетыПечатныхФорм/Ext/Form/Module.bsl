...
Процедура ЗаполнитьТаблицуМакетовПечатныхФорм()
	...
	Для Каждого ОписаниеМакета Из РегистрыСведений.ПоставляемыеМакетыПечати.ВсеМакетыПечатныхФормКонфигурации() Цикл
		...
		//++ Потолки: исключаем неиспользуемые документы
		Если (СтрНайти(ИмяВладельца, "УДАЛИТЬ_") > 0 ИЛИ СтрНайти(Макет.Имя, "УДАЛИТЬ_") > 0) И НЕ ПоказыватьАрхивные Тогда
			Продолжить;
		КонецЕсли;
		//-- Потолки: исключаем неиспользуемые макеты
