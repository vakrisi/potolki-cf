...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли; 
	УстановитьСвойстваПолейПериодов();
...
Процедура СохранитьНастройку(Команда)
	...
	// Связь по идентификатору с таблицей вариантов диаграмм
...
&НаСервере
Процедура УстановитьСвойстваПолейПериодов()
	...
	Иначе
		...
		ИначеЕсли Элемент=Элементы.ПериодСравненияПредставление Тогда
			...
			Если ТипЗнч(Период)=Тип("Структура") ИЛИ ЗначениеЗаполнено(Период.ДатаНачала) Тогда
				ПериодРасчета = УправлениеНебольшойФирмойКлиентСервер.ПредыдущийПлавающийПериод(Период);
				...
				ПериодРасчета = УправлениеНебольшойФирмойКлиентСервер.АналогичныйПериодПрошлогоГода(Период);
