...
Процедура УправлениеФормой()
	...
	Элементы.ГруппаИзменениеЗагружаемогоКурса.Доступность = Объект.ЗагружатьКурсИзИнтернета;
	
	Элементы.ПорядокОкругленияЧислом.Доступность = Объект.ОкруглятьЗагружаемыйКурс;
	Элементы.ОкруглятьВБольшуюСторону.Доступность = Объект.ОкруглятьЗагружаемыйКурс;
	
	Элементы.НаценкаНаЗагружаемыйКурс.Доступность = Объект.ИзменятьЗагружаемыйКурсНаПроцент;
	
КонецПроцедуры

&НаКлиенте
Процедура ОкруглятьЗагружаемыйКурсПриИзменении(Элемент)
	
	Если НЕ Объект.ОкруглятьЗагружаемыйКурс Тогда
		Объект.ПорядокОкругленияЧислом = 0;
...
&НаКлиенте
Процедура ИзменятьЗагружаемыйКурсНаПроцентПриИзменении(Элемент) 	
	
	Если НЕ Объект.ИзменятьЗагружаемыйКурсНаПроцент Тогда
		Объект.НаценкаНаЗагружаемыйКурс = 0;
	КонецЕсли;
	УправлениеФормой();
...
&НаКлиенте
Процедура ЗагружатьКурсИзИнтернетаПриИзменении(Элемент)	
	
	Если НЕ Объект.ЗагружатьКурсИзИнтернета Тогда
		Объект.ИзменятьЗагружаемыйКурсНаПроцент = Ложь;
		ИзменятьЗагружаемыйКурсНаПроцентПриИзменении(Неопределено);
		
		Объект.ОкруглятьЗагружаемыйКурс = Ложь;
		ОкруглятьЗагружаемыйКурсПриИзменении(Неопределено);
	Иначе
		УправлениеФормой();
	КонецЕсли;
...
Процедура ИспользуетсяПриИзменении(Элемент)
	...
	Если НЕ Объект.Используется Тогда
		Объект.ЗагружатьКурсИзИнтернета = Ложь;
		ЗагружатьКурсИзИнтернетаПриИзменении(Неопределено);
	КонецЕсли;
