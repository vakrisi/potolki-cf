...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ #7273 Всегда Сотрудники
	ТипПолучателейРассылки = ТаблицаТиповПолучателей[0].ТипПолучателей;
	Элементы.ТипПолучателейРассылки.Видимость = Ложь;
	Объект.ТипПолучателейРассылки = ТаблицаТиповПолучателей[0].ИдентификаторОбъектаМетаданных;
	Объект.ВидПочтовогоАдресаПолучателей = ТаблицаТиповПолучателей[0].ОсновнойВидКИ;
	//--
	...
	Если Объект.Подготовлена Тогда
		...
		Если Объект.ИспользоватьЭлектроннуюПочту Тогда
			Если Объект.Личная Тогда
				...
				//КонецЕсли;
				Если Не ЗначениеЗаполнено(Объект.ЛичныйEmail) Тогда
					Отказ = Истина;
					ТекстСообщения = НСтр("ru = 'Не указан почтовый адрес'");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Объект.ЛичныйEmail");
				КонецЕсли;
				//-- #7273
			Иначе
...
Процедура ДобавитьОтчет(Команда)
	...
	ПараметрыФормыВыбора.Вставить("РежимПодбора",        Истина);
	//-- #7273
