...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//++ Потолки
	Если НЕ ОграничиватьДоступПоКассамИБанковскимСчетамОрганизаций Тогда
		РеквизитПроверки = ПроверяемыеРеквизиты.Найти("ДоступныеКассы.Касса");
		Если НЕ РеквизитПроверки = Неопределено Тогда
			ПроверяемыеРеквизиты.Удалить(РеквизитПроверки);
		КонецЕсли;
		
		РеквизитПроверки = ПроверяемыеРеквизиты.Найти("ДоступныеБанковскиеСчета.БанковскийСчет");
		Если НЕ РеквизитПроверки = Неопределено Тогда
			ПроверяемыеРеквизиты.Удалить(РеквизитПроверки);
		КонецЕсли;
		
		//#13742
		РеквизитПроверки = ПроверяемыеРеквизиты.Найти("ВидОграниченияДоступаПоКассамИБанковскимСчетам");
		Если НЕ РеквизитПроверки = Неопределено Тогда
			ПроверяемыеРеквизиты.Удалить(РеквизитПроверки);
		КонецЕсли;
	...
	Иначе
		...
		Для Каждого СтрокаТЧ Из ДоступныеКассы Цикл
			РезультатПоиска = ДоступныеКассы.НайтиСтроки(Новый Структура("Касса", СтрокаТЧ.Касса));
			Если РезультатПоиска.Количество()  > 1 Тогда				
				ТекстОшибки = "Обнаружена дублирующаяся строка для позиции: " + СтрокаТЧ.Касса;
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, "ДоступныеКассы["+Формат(ДоступныеКассы.Индекс(СтрокаТЧ), "ЧГ=0")+"].Касса", , Отказ);
			КонецЕсли;
		КонецЦикла;	
		Для Каждого СтрокаТЧ Из ДоступныеБанковскиеСчета Цикл
			РезультатПоиска = ДоступныеБанковскиеСчета.НайтиСтроки(Новый Структура("БанковскийСчет", СтрокаТЧ.БанковскийСчет));
			Если РезультатПоиска.Количество()  > 1 Тогда				
				ТекстОшибки = "Обнаружена дублирующаяся строка для позиции: " + СтрокаТЧ.БанковскийСчет;
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, "ДоступныеБанковскиеСчета["+Формат(ДоступныеБанковскиеСчета.Индекс(СтрокаТЧ), "ЧГ=0")+"].БанковскийСчет", , Отказ);
			КонецЕсли;
		КонецЦикла;	
		
	КонецЕсли;
	
	Если НЕ ОграничиватьДоступПоОрганизациям Тогда
		РеквизитПроверки = ПроверяемыеРеквизиты.Найти("ДоступныеОрганизации.Организация");
		Если НЕ РеквизитПроверки = Неопределено Тогда
			ПроверяемыеРеквизиты.Удалить(РеквизитПроверки);
		КонецЕсли;
	...
	Иначе
		// проверка на дублирующиеся строки
		Для Каждого СтрокаТЧ Из ДоступныеОрганизации Цикл
			РезультатПоиска = ДоступныеОрганизации.НайтиСтроки(Новый Структура("Организация", СтрокаТЧ.Организация));
			Если РезультатПоиска.Количество()  > 1 Тогда				
				ТекстОшибки = "Обнаружена дублирующаяся строка для позиции: " + СтрокаТЧ.Организация;
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, "ДоступныеОрганизации["+Формат(ДоступныеОрганизации.Индекс(СтрокаТЧ), "ЧГ=0")+"].Организация", , Отказ);
			КонецЕсли;
		КонецЦикла;	
	КонецЕсли;
	
	Если НЕ ОграничиватьДоступПоСкладам Тогда
		РеквизитПроверки = ПроверяемыеРеквизиты.Найти("ДоступныеСклады.Склад");
		Если НЕ РеквизитПроверки = Неопределено Тогда
			ПроверяемыеРеквизиты.Удалить(РеквизитПроверки);
		КонецЕсли;		
	КонецЕсли;
	
	Если НЕ ОграничиватьПереченьДоступныхВидовЦен Тогда
		РеквизитПроверки = ПроверяемыеРеквизиты.Найти("ДоступныеВидыЦен.ВидЦены");
		Если НЕ РеквизитПроверки = Неопределено Тогда
			ПроверяемыеРеквизиты.Удалить(РеквизитПроверки);
		КонецЕсли;
	КонецЕсли;
	
	Если НЕ ОграничиватьДоступККонтрагентамПоГруппамДоступа Тогда
		РеквизитПроверки = ПроверяемыеРеквизиты.Найти("ДоступныеГруппыКонтрагентов.ГруппаКонтрагентов");
		Если НЕ РеквизитПроверки = Неопределено Тогда
			ПроверяемыеРеквизиты.Удалить(РеквизитПроверки);
		КонецЕсли;
	Иначе	
		// проверка на дублирующиеся строки
		Для Каждого СтрокаТЧ Из ДоступныеГруппыКонтрагентов Цикл
			РезультатПоиска = ДоступныеГруппыКонтрагентов.НайтиСтроки(Новый Структура("ГруппаКонтрагентов", СтрокаТЧ.ГруппаКонтрагентов));
			Если РезультатПоиска.Количество()  > 1 Тогда				
				ТекстОшибки = "Обнаружена дублирующаяся строка для позиции: " + СтрокаТЧ.ГруппаКонтрагентов;
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, "ДоступныеГруппыКонтрагентов["+Формат(ДоступныеГруппыКонтрагентов.Индекс(СтрокаТЧ), "ЧГ=0")+"].ГруппаКонтрагентов", , Отказ);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Если НЕ ОграничиватьПереченьДоступныхКатегорийКонтрагентов Тогда
		РеквизитПроверки = ПроверяемыеРеквизиты.Найти("ДоступныеКатегорииКонтрагентов.Категория");
		Если НЕ РеквизитПроверки = Неопределено Тогда
			ПроверяемыеРеквизиты.Удалить(РеквизитПроверки);
		КонецЕсли;
	КонецЕсли;
	
	Если НЕ ОграничиватьДоступПоЭквайринговымТерминалам Тогда
		РеквизитПроверки = ПроверяемыеРеквизиты.Найти("ДоступныеЭТ.Терминал");
		Если НЕ РеквизитПроверки = Неопределено Тогда
			ПроверяемыеРеквизиты.Удалить(РеквизитПроверки);
		КонецЕсли;		
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	ОбщийМодульСервер.ВыполнитьПроверкуПоследнегоАдминистратораПользователя(ЭтотОбъект); // №12464
...
Процедура ПередУдалением(Отказ)
	...
	//++ Потолки
	РегистрыСведений.ИсторияЗвонковАТС.ОчиститьПользователяВЗаписях(Ссылка);
	РегистрыСведений.НомераРасчетов.ОчиститьПользователяВЗаписях(Ссылка);
	Справочники.ЭтапыВнедрения.ОчиститьПользователяВЗаписях(Ссылка);
	//-- Потолки
