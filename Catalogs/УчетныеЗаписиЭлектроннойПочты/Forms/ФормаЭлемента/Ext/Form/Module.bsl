...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++
	Элементы.СтраницаМаркетинг.Видимость = Константы.СоздаватьВходящиеКонтактыПоВходящимЭлектроннымПисьмам.Получить();
	//--
...
Процедура ПроверитьНастройки(Команда)
	...
	СообщениеРезультата = ПроверитьНастройкиНаСервере();
	ПоказатьОповещениеПользователя(СообщениеРезультата);
	//--
...
Функция ПроверитьНастройкиНаСервере()
	...
	Профиль = Справочники.УчетныеЗаписиЭлектроннойПочты.ПолучитьИнтернетПочтовыйПрофиль(Объект); 
	
	ИнтернетПочта = Новый ИнтернетПочта;
	
	Попытка
		ИнтернетПочта.Подключиться(Профиль, ?(объект.ИспользоватьIMAP, ПротоколИнтернетПочты.IMAP, ПротоколИнтернетПочты.POP3));
	Исключение
		СообщениеРезультата = "ОШИБКА! Не удалось подключиться к почтовому серверу. Описание ошибки: " + ОписаниеОшибки();
		Сообщить(СообщениеРезультата);
		Возврат "ОШИБКА!";
	КонецПопытки;
	
	СообщениеРезультата = "Все работает!";
	Возврат СообщениеРезультата;
...
Процедура ОбновитьОтображениеЭлементовФормы()
	...
	Элементы.ГруппаНастройкиМаркетинг.Видимость = Объект.СоздаватьВходящиеКонтакты;

	Элементы.ГруппаПояснения.Видимость = ?(СтрНайти(Объект.АдресСервераPOP3,"gmail")>0,Истина,Ложь);
	
КонецПроцедуры

&НаКлиенте
Процедура ИспользоватьPOP3ПриИзменении(Элемент)
	Объект.ИспользоватьIMAP = НЕ Объект.ИспользоватьPOP3;
	Если Объект.ИспользоватьPOP3 Тогда
		Если Объект.СоздаватьВходящиеКонтакты Тогда
			Объект.СоздаватьВходящиеКонтакты = Ложь;
			Состояние("Автоматическое создание входящих контактов отменено");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ИспользоватьIMAPПриИзменении(Элемент)
	Объект.ИспользоватьPOP3 = НЕ Объект.ИспользоватьIMAP;
КонецПроцедуры

&НаКлиенте
Процедура СоздаватьВходящиеКонтактыПриИзменении(Элемент)
	СоздаватьВходящиеКонтактыПриИзмененииНаСервере();
	ОбновитьОтображениеЭлементовФормы();
КонецПроцедуры

&НаСервере
Процедура СоздаватьВходящиеКонтактыПриИзмененииНаСервере()
	Если Объект.СоздаватьВходящиеКонтакты Тогда
		Если НЕ Объект.ИспользоватьIMAP Тогда
			Объект.СоздаватьВходящиеКонтакты = Ложь;
			Сообщить("Создавать входящие контакты можно только по учетной записи IMAP");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура АдресСервераSMTPПриИзменении(Элемент)
	ОбновитьОтображениеЭлементовФормы();
КонецПроцедуры
...
Процедура ПочтовыйСервисПриИзменении(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ПочтовыйСервис) Тогда
		Возврат;
	КонецЕсли;
	
	ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗакрытияВопросаОПерезаполненииПараметров", ЭтаФорма, Параметры), "Изменился почтовый сервис. Перезаполнить параметры учетной записи?", РежимДиалогаВопрос.ДаНет,,,,);	
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеЗакрытияВопросаОПерезаполненииПараметров(Результат, Параметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		ПерезаполнитьПараметрыУчетнойЗаписи();
		ИспользоватьPOP3ПриИзменении(Неопределено);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ПерезаполнитьПараметрыУчетнойЗаписи()
	РаботаСЭлектроннойПочтой.ЗаполнитьПараметрыУчетнойЗаписи(Объект);
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияСсылкаГуглНажатие(Элемент)
	ЗапуститьПриложение("https://www.google.com/settings/security/lesssecureapps");
КонецПроцедуры

&НаКлиенте
Процедура ПрикреплятьВложенияКПисьмуПриИзменении(Элемент)
	
	Если НЕ Объект.ПрикреплятьВложенияКПисьму Тогда
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = "Внимание! Вложения к письмам могу содержать вредоносное программное обеспечение, 
					|которое может нанести непоправимый ущерб компьютеру. 
					|Так же сохранение вложений к письмам может привести к росту размера базы данных. 
					|Включить прикрепление файлов к входящим письмам?";	
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПрикреплятьВложенияПриИзмененииПослеВопроса", ЭтаФорма, "ПрикреплятьВложенияКПисьму");
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры

&НаКлиенте
Процедура ПрикреплятьВложенияКЛидуПриИзменении(Элемент)
	
	Если НЕ Объект.ПрикреплятьВложенияКЛиду Тогда
		Возврат;
	КонецЕсли;
	...
	ТекстВопроса = "Внимание! Вложения к письмам могу содержать вредоносное программное обеспечение, 
					|которое может нанести непоправимый ущерб компьютеру. 
					|Так же сохранение вложений к письмам может привести к росту размера базы данных. 
					|Включить прикрепление файлов к лидам?";	
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПрикреплятьВложенияПриИзмененииПослеВопроса", ЭтаФорма, "ПрикреплятьВложенияКЛиду");
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры

&НаКлиенте
Процедура ПрикреплятьВложенияПриИзмененииПослеВопроса(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Нет Тогда
		ИмяРеквизита = ДополнительныеПараметры;
