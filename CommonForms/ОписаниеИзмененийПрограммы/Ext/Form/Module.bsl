...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Разделы.Количество() = 0 
		И НЕ Параметры.Свойство("ОткрытиеИзГлавногоМеню") // #13731 при открытии из главного меню должна отработать обычная логика: отображение всех изменений программы
		Тогда
		МассивНомеровВерсий = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Метаданные.Версия, ".");
		Если НЕ МассивНомеровВерсий[2] = "0" Тогда
			ВерсияРодитель = МассивНомеровВерсий[0] + "." + МассивНомеровВерсий[1] + ".0.0";
			Разделы.Добавить(ВерсияРодитель);
		КонецЕсли;
	КонецЕсли; 
	//--
