...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.КоллекцияПечатныхФорм = Неопределено Тогда
		...
		//++ Потолки
		Если Параметры.Свойство("ПотолкиПечатьШтрихкода") Тогда
			Элементы.ГруппаКоманднаяПанельПечатьШК.Видимость = Истина;
			ПотолкиТипШтрихкода = Параметры.ПотолкиТипШтрихкода;
			ПотолкиЗначениеКода = Параметры.ПотолкиЗначениеКода; 
			ПотолкиЗаголовокКода = Параметры.ПотолкиЗаголовокКода;
			//КоллекцияПечатныхФорм = УправлениеПечатьюВызовСервера.НоваяКоллекцияПечатныхФорм("Штрихкод");
			КоллекцияПечатныхФорм = УправлениеПечатью.ПодготовитьКоллекциюПечатныхФорм("Штрихкод");
			ПечатнаяФормаШК = КоллекцияПечатныхФорм[0]; //УправлениеПечатьюКлиент.ОписаниеПечатнойФормы(КоллекцияПечатныхФорм, "Штрихкод");
			ПечатнаяФормаШК.СинонимМакета = "Штрихкод";
			ПечатнаяФормаШК.ТабличныйДокумент = ОбщийМодульВызовСервера.ПолучитьШтрихКодНаПечать(ПотолкиТипШтрихкода, ПотолкиЗначениеКода, ПотолкиЗаголовокКода);
			ПечатнаяФормаШК.ИмяФайлаПечатнойФормы = "Штрихкод";	
		Иначе
		//-- Потолки
		...
		Если Параметры.ПараметрыПечати.Свойство("ОбъектыПечати") Тогда 
			ДокументыДляПечати = Параметры.ПараметрыПечати.ОбъектыПечати;
			Если ТипЗнч(ДокументыДляПечати) = Тип("Массив") Тогда
				Для ТекНомер = 0 По ДокументыДляПечати.Количество()-1 Цикл
					ПотолкиОбъектПечати = ДокументыДляПечати[ТекНомер];					
					Попытка 
						Контрагент = ДокументыДляПечати[ТекНомер].Контрагент;
					Исключение
					КонецПопытки;
					Если ТипЗнч(ДокументыДляПечати[ТекНомер]) = Тип("ДокументСсылка.Лид") Тогда
						Контрагент = Новый Структура("Наименование", ДокументыДляПечати[ТекНомер].ИмяКлиента);
						Контрагент.Вставить("ОсновнойEMail", УправлениеКонтактнойИнформациейПотолки.ПолучитьАдресПочтыИзКонтактнойИнформацииОбъекта(ДокументыДляПечати[ТекНомер], Истина));
					КонецЕсли;
					
					Попытка
						ЗаголовокПечатнойФормы = Строка(ДокументыДляПечати[ТекНомер].Ссылка);
						Поз = СтрНайти(ЗаголовокПечатнойФормы, " ", НаправлениеПоиска.СКонца) - 1;
						Если Поз > 0 Тогда
							ЗаголовокПечатнойФормы = Лев(ЗаголовокПечатнойФормы, Поз);
						КонецЕсли;
						
						ТипСсылки = ТипЗнч(ДокументыДляПечати[ТекНомер].Ссылка);						
						Если Документы.ТипВсеСсылки().СодержитТип(ТипСсылки) Тогда
							ЗаголовокПечатнойФормы = СтрЗаменить(ЗаголовокПечатнойФормы, Строка(ДокументыДляПечати[ТекНомер].Номер), ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДокументыДляПечати[ТекНомер].Номер));
						КонецЕсли;
				...
				КонецЦикла
			КонецЕсли;
		ИначеЕсли Параметры.ПараметрыИсточника <> Неопределено И Параметры.ПараметрыИсточника.Свойство("ОбъектыНазначения") Тогда
			ДокументыДляПечати = Параметры.ПараметрыИсточника.ОбъектыНазначения;
			Если ТипЗнч(ДокументыДляПечати) = Тип("Массив") Тогда
				Для ТекНомер = 0 По ДокументыДляПечати.Количество()-1 Цикл
					ПотолкиОбъектПечати = ДокументыДляПечати[ТекНомер];
					Попытка 
			...
			КонецЕсли;
		КонецЕсли;
		Если Параметры.ПараметрыПечати.ДополнительныеПараметры.Свойство("ИмяВложенияИзЗаголовкаФормы") И Параметры.ПараметрыПечати.Свойство("ЗаголовокФормы") Тогда
			Если Параметры.ПараметрыПечати.ДополнительныеПараметры.ИмяВложенияИзЗаголовкаФормы И ЗначениеЗаполнено(Параметры.ПараметрыПечати.ЗаголовокФормы) Тогда
				ЗаголовокПечатнойФормы = Параметры.ПараметрыПечати.ЗаголовокФормы;
			КонецЕсли;
		КонецЕсли;
		Если Параметры.ПараметрыПечати.Свойство("ЗаголовокПечатнойФормы") Тогда
			Если НЕ ПустаяСтрока(Параметры.ПараметрыПечати.ЗаголовокПечатнойФормы) Тогда
				ЗаголовокПечатнойФормы = Параметры.ПараметрыПечати.ЗаголовокПечатнойФормы;
			КонецЕсли;
		КонецЕсли;
	...
	ВыбранныйПринтер = ?(ПустаяСтрока(ВыбранныйПринтер), "(По умолчанию)", ВыбранныйПринтер);
	Элементы.ВыбранныйПринтер.СписокВыбора.Добавить(ВыбранныйПринтер);
	//--
...
Процедура Отправить(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОтправитьПослеВопроса", ЭтотОбъект, );
...
Процедура СоздатьРеквизитыИЭлементыФормыДляПечатныхФорм(КоллекцияПечатныхФорм)
	...
	Для Каждого РеквизитФормы Из НовыеРеквизитыФормы Цикл
		...
		Если РанееДобавленнаяНастройкаПечатнойФормы = Неопределено Тогда
			...
			НовыйЭлемент.Защита = ОписаниеПечатнойФормы.ТабличныйДокумент.Защита; //+- Потолки Или Не Пользователи.РолиДоступны("РедактированиеПечатныхФорм");
...
Процедура УстановитьИмяПринтераВПодсказкеКнопкиПечать(ИмяПринтера = Неопределено)
	Если НастройкиПечатныхФорм.Количество() > 0 Тогда
		Если ИмяПринтера = Неопределено Тогда //+- потолки
...
Функция ТабличныеДокументыДляПечати()
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		Если Элементы[НастройкаПечатнойФормы.ИмяРеквизита].Вывод = ИспользованиеВывода.Разрешить И НастройкаПечатнойФормы.Печатать Тогда
			...
			//++
			// берем указанный пользователем принтер
			ТабличныйДокумент.ИмяПринтера = ?(ВыбранныйПринтер = "(По умолчанию)", "", ВыбранныйПринтер);
			//--
...
Функция ПолучитьМассивПринтеров()
    ...
    Выборка = objWMIService.ExecQuery("SELECT Caption FROM Win32_Printer",,48);    
...
&НаКлиенте
Процедура ВыбранныйПринтерНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Если Элементы.ВыбранныйПринтер.СписокВыбора.Количество() = 1 Тогда		
		МассивПринтеров = ПолучитьМассивПринтеров();
		Элементы.ВыбранныйПринтер.СписокВыбора.ЗагрузитьЗначения(МассивПринтеров);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ВыбранныйПринтерПриИзменении(Элемент)
	ВыбранныйПринтер = ?(ПустаяСтрока(ВыбранныйПринтер), "(По умолчанию)", ВыбранныйПринтер);
	УстановитьИмяПринтераВПодсказкеКнопкиПечать(ВыбранныйПринтер);
КонецПроцедуры

&НаКлиенте
Процедура ПотолкиТипШтрихкодаПриИзменении(Элемент)
	ПотолкиТипШтрихкодаПриИзмененииНаСервере();
КонецПроцедуры

&НаСервере
Процедура ПотолкиТипШтрихкодаПриИзмененииНаСервере()
	ИмяРеквизита = "ПечатнаяФорма1";
	ЭтотОбъект[ИмяРеквизита] = ОбщийМодульВызовСервера.ПолучитьШтрихКодНаПечать(ПотолкиТипШтрихкода, ПотолкиЗначениеКода, ПотолкиЗаголовокКода);
	ТекущаяПечатнаяФорма = ЭтотОбъект[ИмяРеквизита];
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьПослеВопроса(Результат, ДополнительныеПараметры) Экспорт
...
Функция СформироватьЭлектронноеПисьмоСОтчетом()
	...
	ЗаголовокПисьмаИИмяФайла = ЗаголовокПечатнойФормы;
	//#14250
	Если НастройкиПечатныхФорм.Количество() > 0 И ОбъектыПечати.Количество() > 0 Тогда
		НастройкаПечатнойФормы = НастройкиПечатныхФорм[0];
		ОбъектПечати = ОбъектыПечати[0].Значение;
	 	ЗаданныеИменаПечатныхФорм = ОбщегоНазначения.ЗначениеИзСтрокиXML(НастройкаПечатнойФормы.ИмяФайлаПечатнойФормы);
		ЗаголовокПисьмаИИмяФайла = УправлениеПечатью.ИмяФайлаПечатнойФормыОбъекта(ОбъектПечати, ЗаданныеИменаПечатныхФорм, НастройкаПечатнойФормы.Название);
		ЗаголовокПисьмаИИмяФайла = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ЗаголовокПисьмаИИмяФайла);
	КонецЕсли;
	
	СтруктураПисьма = Новый Структура;
	СтруктураПисьма.Вставить("Основание", ПотолкиОбъектПечати);
	Возврат РаботаСЭлектроннойПочтой.СформироватьЭлектронноеПисьмоСОтчетом(ТекущаяПечатнаяФорма, ЗаголовокПисьмаИИмяФайла, Контрагент, "pdf", , СтруктураПисьма);
