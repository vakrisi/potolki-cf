...
Процедура ОК(Команда)
	...
	Оповестить("ИА_ВыборЦвета", Элементы.ПолеКартинкиЦвета.ЦветФона); 
	Закрыть();
КонецПроцедуры

&НаКлиенте
Процедура ВыборСтандартногоЦвета(Команда)
	ЗначениеЦвета = СтрЗаменить(Команда.Имя,"Цвет","");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МаксЭлементовВСтроке = 15;
	ГруппаСтрока = Неопределено;
	
	
	ЭлементовВСтроке = 0;
	
	шрифтКнопок = Новый Шрифт(,6,Ложь);
	
	Для Каждого Стр из ТабличноеПоле Цикл
		
		ЭлементовВСтроке = ЭлементовВСтроке + 1;
		Если (ЭлементовВСтроке%МаксЭлементовВСтроке = 0) ИЛИ (ГруппаСтрока=Неопределено) Тогда
			ГруппаСтрока = Элементы.Добавить("ГрЦвет"+ЭлементовВСтроке, Тип("ГруппаФормы"), Элементы.ГруппаСтандартныеЦвета);
			ГруппаСтрока.Вид = ВидГруппыФормы.ОбычнаяГруппа;
			ГруппаСтрока.Заголовок = "";
		...
		кмд = Команды.Добавить("Цвет"+Стр.ЗначениеЦвета);
		кмд.Действие = "ВыборСтандартногоЦвета";
	    кмд.Подсказка = Стр.НазваниеЦвета;
		
		кн = Элементы.Добавить(кмд.Имя, Тип("КнопкаФормы"), ГруппаСтрока);
		кн.Заголовок = " ";
		
		
		_Красный = Число(Лев(Стр.ЗначениеЦвета, 3));
		_Зеленый = Число(Сред(Стр.ЗначениеЦвета, 4, 3));
		_Синий = Число(Прав(Стр.ЗначениеЦвета, 3));

		кн.ЦветФона = Новый Цвет(_Красный,_Зеленый,_Синий);
		кн.Шрифт = шрифтКнопок;
		кн.ИмяКоманды = кмд.Имя;
		кн.Ширина = 2;
		кн.Высота = 1;
		
	КонецЦикла;
	
	Элементы.ПолеКартинкиЦвета.ЦветФона = Параметры.ИсходныйЦвет;
	Красный = Параметры.ИсходныйЦвет.Красный;
	Зеленый = Параметры.ИсходныйЦвет.Зеленый;
	Синий = Параметры.ИсходныйЦвет.Синий;
