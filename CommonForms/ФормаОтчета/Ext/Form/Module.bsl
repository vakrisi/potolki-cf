...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ Потолки: меню всегда отображаем 
	...
	//++ Потолки
	УправлениеФормойВосстановлениеПоследовательности();

	РегулируетсяОтборПоОрганизации = (НастройкиОтчета.ПолноеИмя = "Отчет.АнализДоступностиТоваров")
	                             ИЛИ (НастройкиОтчета.ПолноеИмя = "Отчет.ПродажиИОстатки");
	
	УстановитьВидимостьОтбораПоОрганизации();	
	
	Если НастройкиОтчета.ПолноеИмя = "Отчет.ДниРождения" Тогда
		ОтчетОбъект.ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка);
	КонецЕсли;
	
	//++ №9821 справка на сайте (указывается часть URL статьи)
	СправкиНаСайте = новый Структура;
	СправкиНаСайте.Вставить("ВаловаяПрибыль", "/ru/knowledge_base/article/233/category/30/");
	СправкиНаСайте.Вставить("ВаловаяПрибыльПроизводство", "/ru/knowledge_base/article/235/category/30/");
	СправкиНаСайте.Вставить("ВаловаяПрибыльРозница", "/ru/knowledge_base/article/234/category/30/");
	
	Элементы.СправкаНаСайте.Видимость = (СправкиНаСайте.Свойство(ОтчетМетаданные.Имя));
	Элементы.Справка.Видимость = НЕ Элементы.СправкаНаСайте.Видимость;
	//-- №9821
	
	//++ №14538
	Элементы.ДополнительныеПоляОтчётаДетализацияВзаиморасчетов.Видимость = (НастройкиОтчета.ПолноеИмя = "Отчет.ДетализацияВзаиморасчетовСКонтрагентом");
	
	Если ПолучитьФункциональнуюОпцию("УчетБонусов") Тогда
		СписокДополнительныхПолейОтчётаДетализацияВзаиморасчетов.Добавить("Бонусы");
		//СписокДополнительныхПолейОтчётаДетализацияВзаиморасчетов.Добавить("СуммаБонусовРасход", "Списание бонусов");
	КонецЕсли;
	СписокДополнительныхПолейОтчётаДетализацияВзаиморасчетов.Добавить("АдресМонтажа", "Адрес монтажа");
	СписокДополнительныхПолейОтчётаДетализацияВзаиморасчетов.Добавить("Комментарий");
	//-- №14538
	//-- Потолки
...
Процедура ПриОткрытии(Отказ)
	...
	//++
	УстановитьОформлениеПользовательскихНастроек(); // №13736
	//-- 
	
	//nva+++
	Если НастройкиОтчета.ПолноеИмя = "Отчет.ЗагруженностьПроизводства" ИЛИ НастройкиОтчета.ПолноеИмя = "Отчет.ЗагруженностьПроизводстваКратко" Тогда
...
Процедура ПриЗагрузкеВариантаНаСервере(НовыеНастройкиКД)
	...
	МодульОтчетов.ДействияПриЗагрузкеПользовательскихНастроекНаСервере(Отчет, ИмяОтчета);
	УстановитьВидимостьОтбораПоОрганизации();
	//--
	...
	//#9159
	Если НастройкиОтчета.ПолноеИмя = "Отчет.РасчетыПоЗаказам" Тогда
		ОтчетОбъект = РеквизитФормыВЗначение("Отчет");
		ОтчетОбъект.ПриЗагрузкеВариантаНаСервере(ЭтотОбъект, НовыеНастройкиКД);
	КонецЕсли;
	
	//++ №13246
	Если НастройкиОтчета.ПолноеИмя = "Отчет.ДетализацияВзаиморасчетовСКонтрагентом" Тогда
		
		ОтчетОбъект = РеквизитФормыВЗначение("Отчет");
		ОтчетОбъект.ПроверитьОтключитьКолонкиБонусы(Отчет.КомпоновщикНастроек.Настройки);
		
		ЗаполнитьДополнительныеПоляОтчётаДетализацияВзаиморасчетов(); // №14538
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(НовыеПользовательскиеНастройкиКД)
	...
	//-- Потолки
	МодульОтчетов.ДействияПриЗагрузкеПользовательскихНастроекНаСервере(Отчет,РеквизитФормыВЗначение("Отчет").Метаданные().Имя);	
	УстановитьВидимостьОтбораПоОрганизации();
	//--
...
Процедура ПриОбновленииСоставаПользовательскихНастроекНаСервере(СтандартнаяОбработка)
	...
	//++
	Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("КлючВарианта", КлючТекущегоВарианта);
	//--
...
Процедура ПриСохраненииПользовательскихНастроекНаСервере(ПользовательскиеНастройкиКД)
	...
	//++ Потолки
	МодульОтчетов.ДействияПриСохраненииПользовательскихНастроекНаСервере(Отчет,РеквизитФормыВЗначение("Отчет").Метаданные().Имя, ПользовательскиеНастройкиКД);	
	//--
КонецПроцедуры
...
Процедура ОтчетСкомпоноватьРезультат(Команда)
	...
	//++
	Потолки_ПередКомпоновкойРезультата();
	//--
...
Процедура УстановитьВидимостьДоступность(ПриСохраненииВарианта = Ложь)
	...
	Если ЗначениеЗаполнено(ОтчетНаименованиеТекущегоВарианта) Тогда
		...
		//++ Потолки
		Заголовок = СтрШаблон("%1 (%2)", НастройкиОтчета.Наименование, ОтчетНаименованиеТекущегоВарианта);
		//-- Потолки
	Иначе
...
&НаСервере
Процедура УправлениеФормойВосстановлениеПоследовательности()
		
	Если НастройкиОтчета.ПолноеИмя = "Отчет.ВаловаяПрибыль"
		ИЛИ НастройкиОтчета.ПолноеИмя = "Отчет.ВаловаяПрибыльПроизводство"
		ИЛИ НастройкиОтчета.ПолноеИмя = "Отчет.ВаловаяПрибыльРозница"
		ИЛИ (НастройкиОтчета.ПолноеИмя = "Отчет.ВедомостьПоТоварамНаСкладах" И КлючТекущегоВарианта = "Стоимостная оценка товаров на складах") // №12984 было: КлючТекущегоВарианта = "Ведомость по товарам на складах с себестоимостью")		
		ИЛИ (НастройкиОтчета.ПолноеИмя = "Отчет.ВедомостьПоТоварамНаСкладах" И КлючТекущегоВарианта = "Ведомость по товарам на складах с себестоимостью") //#14965
		ИЛИ НастройкиОтчета.ПолноеИмя = "Отчет.ИзлишкиИНедостачи" 
		ИЛИ НастройкиОтчета.ПолноеИмя = "Отчет.МатериалыВЭксплуатации"
		Тогда
	...
	// №13736
	Элементы.ДекорацияСводныйУправленческийОтчетПроизводства.Видимость = (НастройкиОтчета.ПолноеИмя = "Отчет.СводныйУправленческийОтчетПроизводства");
...
//++ №14538
&НаКлиенте
Процедура ДополнительныеПоляОтчётаДетализацияВзаиморасчетовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ДополнительныеПоляОтчётаДетализацияВзаиморасчетовЗавершениеВыбора", ЭтотОбъект);
	ЗаголовокДиалога = НСтр("ru = 'Выберите дополнительные поля отчёта'");
	
	СписокДополнительныхПолейОтчётаДетализацияВзаиморасчетов.ПоказатьОтметкуЭлементов(ОписаниеОповещения, ЗаголовокДиалога);
	
КонецПроцедуры

&НаКлиенте
Процедура ДополнительныеПоляОтчётаДетализацияВзаиморасчетовЗавершениеВыбора(ВыбранныеПоля, Переменные) Экспорт

	Если ВыбранныеПоля = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ДополнительныеПоляОтчётаДетализацияВзаиморасчетов.Очистить();
	
	Для каждого КлючЗначение из ВыбранныеПоля Цикл
		Если КлючЗначение.Пометка Тогда
			ДополнительныеПоляОтчётаДетализацияВзаиморасчетов.Добавить(КлючЗначение.Значение, КлючЗначение.Представление);
		КонецЕсли;
	КонецЦикла;
	
	НастройкиКомпоновщикНастроек = Отчет.КомпоновщикНастроек.Настройки;
	
	Для Каждого ЭлКолонки Из НастройкиКомпоновщикНастроек.Выбор.Элементы Цикл
		
		НазваниеПоля = Строка(ЭлКолонки.Поле);
		Если НазваниеПоля = "СуммаБонусовПриход"
			Или НазваниеПоля = "СуммаБонусовРасход"
			//++ №14592
			Или НазваниеПоля = "ОР_БонусыСуммаНачальныйОстаток"
			Или НазваниеПоля = "СуммаБонусовНачальныйОстаток"
			Или НазваниеПоля = "СуммаБонусовКонечныйОстаток"
			Или НазваниеПоля = "ОР_БонусыСуммаКонечныйОстаток"
			//-- №14592
		Тогда
			НазваниеПоля = "Бонусы";
		КонецЕсли;
		
		// если поле есть в общем спике возможных полей
		Если НЕ СписокДополнительныхПолейОтчётаДетализацияВзаиморасчетов.НайтиПоЗначению(НазваниеПоля) = Неопределено Тогда
			
			// фиксируем использование по наличию в списке выбранных полей
			ЭлКолонки.Использование = НЕ (ДополнительныеПоляОтчётаДетализацияВзаиморасчетов.НайтиПоЗначению(НазваниеПоля) = Неопределено);
...
&НаСервере
Процедура ЗаполнитьДополнительныеПоляОтчётаДетализацияВзаиморасчетов()
	
	Для Каждого ЭлКолонки Из Отчет.КомпоновщикНастроек.Настройки.Выбор.Элементы Цикл
		...
		ДопПоле = СписокДополнительныхПолейОтчётаДетализацияВзаиморасчетов.НайтиПоЗначению(НазваниеПоля);
		
		Если НЕ ДопПоле = Неопределено И ЭлКолонки.Использование Тогда
			
			ДопПоле.Пометка = Истина;
			
			Если ДополнительныеПоляОтчётаДетализацияВзаиморасчетов.НайтиПоЗначению(НазваниеПоля) = Неопределено Тогда
				ДополнительныеПоляОтчётаДетализацияВзаиморасчетов.Добавить(ДопПоле.Значение, ДопПоле.Представление);
			КонецЕсли;
...
Процедура УстановитьВидимостьОтбораПоОрганизации()
	...
	Если НЕ РегулируетсяОтборПоОрганизации тогда
		Возврат;
	КонецЕсли;

	ИспользуютсяОбщиеСклады = Константы.ОбщиеСклады.Получить();
	РежимОтображения = ?(ИспользуютсяОбщиеСклады, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ);
	...
	Если ИдентификаторПользовательскойНастройки <> Неопределено И ЗначениеЗаполнено(ИдентификаторПользовательскойНастройки) Тогда
		...
		Если ИспользуютсяОбщиеСклады Тогда
...
&НаСервере
Функция Потолки_ПередКомпоновкойРезультата()
	
	//#12365
	ИмяОтчета = РеквизитФормыВЗначение("Отчет").Метаданные().Имя;
	ПользовательскиеНастройки = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки;
	МодульОтчетов.ДействияПриСохраненииПользовательскихНастроекНаСервере(Отчет, ИмяОтчета, ПользовательскиеНастройки);
	
КонецФункции

&НаКлиенте
Процедура ОткрытьСправкуНаСайте(Команда)
	
	ЗапуститьПриложение("https://help.smartceilings.ru" + СправкиНаСайте[НастройкиОтчета.Имя]);
	
КонецПроцедуры

&НаКлиенте
Процедура Потолки_ОтправитьПоПочте(Команда)
	
	РасширениеВложения = "xls";
	Если Команда.Имя = "ОтправитьПоПочтеPDF" Тогда
		РасширениеВложения = "pdf";
	КонецЕсли;
	
	ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
	Если ОтображениеСостояния.Видимость = Истина Тогда
	//Если ПолучитьКоличествоСтраницВОтчете() = 0 Тогда
		ПоказатьОповещениеПользователя("Сначала необходимо сформировать отчет");
		Возврат;
	КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ОтправитьПоПочтеЗавершение", ЭтотОбъект, РасширениеВложения);
...
Процедура ОтправитьПоПочтеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СсылкаНаЭлектронноеПисьмо = СформироватьЭлектронноеПисьмоСОтчетом(ДополнительныеПараметры);
	ПараметрыПисьма = Новый Структура("Ключ", СсылкаНаЭлектронноеПисьмо);
	ОткрытьФорму("Документ.ЭлектронноеПисьмо.ФормаОбъекта", ПараметрыПисьма);		
	
КонецПроцедуры

&НаСервере
Функция СформироватьЭлектронноеПисьмоСОтчетом(РасширениеВложения)
	
	ТемаПисьма = НСтр("ru = 'Отправка отчета по почте'");
	Контрагент = Справочники.Контрагенты.ПустаяСсылка();
	НачалоПериодаДата = Дата(1,1,1);
	КонецПериодаДата  = Дата(1,1,1);
	
	Если НастройкиОтчета.ПолноеИмя = "Отчет.ДетализацияВзаиморасчетовСКонтрагентом" 
		ИЛИ НастройкиОтчета.ПолноеИмя = "Отчет.ВедомостьПоВзаиморасчетамСКонтрагентами"		
		ИЛИ НастройкиОтчета.ПолноеИмя = "Отчет.ВедомостьПоВзаиморасчетамСКонтрагентамиВВалюте" 
		ИЛИ НастройкиОтчета.ПолноеИмя = "Отчет.АнализЗаказовНаПроизводство"
		Тогда
		//Контрагент
		Попытка
			ИдНастройкиКонтрагент = "";
			...
			Для каждого ЭлементОтбора Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
				Если ЭлементОтбора.ЛевоеЗначение = ПолеКонтрагент Тогда
					ИдНастройкиКонтрагент = ЭлементОтбора.ИдентификаторПользовательскойНастройки;
				КонецЕсли; 
			КонецЦикла;
			Если НЕ ИдНастройкиКонтрагент = "" Тогда
				КонтрагентСКД = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(ИдНастройкиКонтрагент);
				Если НЕ КонтрагентСКД = Неопределено Тогда
					Если КонтрагентСКД.Использование Тогда
						Контрагент = КонтрагентСКД.ПравоеЗначение;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		Исключение
		...
		Попытка
			...
			Если НачалоПериода.Использование Тогда
				НачалоПериодаДата = НачалоПериода.Значение.Дата;
			КонецЕсли;
		Исключение
		КонецПопытки;	
		//КонецПериода
		Попытка
			...
			Если КонецПериода.Использование Тогда
				КонецПериодаДата = КонецПериода.Значение.Дата;
			КонецЕсли;	
		Исключение
		КонецПопытки;
		...
		Если НачалоПериода = Неопределено И КонецПериода = Неопределено Тогда
			Попытка
				ИдНастройкиПериода = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("СтПериод").ИдентификаторПользовательскойНастройки;
				СтПериод = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(ИдНастройкиПериода);
				Если НЕ СтПериод = Неопределено И СтПериод.Использование Тогда
					НачалоПериодаДата = СтПериод.Значение.ДатаНачала;
					КонецПериодаДата  = СтПериод.Значение.ДатаОкончания;
				КонецЕсли;
			Исключение
			КонецПопытки;
		КонецЕсли;
	...
	Если НастройкиОтчета.ПолноеИмя = "Отчет.ДетализацияВзаиморасчетовСКонтрагентом" 
		ИЛИ НастройкиОтчета.ПолноеИмя = "Отчет.ВедомостьПоВзаиморасчетамСКонтрагентами"	Тогда
		ТемаПисьма = "Акт сверки с " + Формат(НачалоПериодаДата, "ДЛФ=Д") + " по " + Формат(КонецПериодаДата, "ДЛФ=Д");
		
	ИначеЕсли НастройкиОтчета.ПолноеИмя = "Отчет.ВедомостьПоВзаиморасчетамСКонтрагентамиВВалюте" Тогда	
		ТемаПисьма = "Ведомость взаиморасчетов в валюте";
		
	ИначеЕсли НастройкиОтчета.ПолноеИмя = "Отчет.АнализЗаказовНаПроизводство" Тогда
		ТемаПисьма = "Анализ заказов на производство сверки с " + Формат(НачалоПериодаДата, "ДЛФ=Д") + " по " + Формат(КонецПериодаДата, "ДЛФ=Д");
		
	ИначеЕсли НастройкиОтчета.ПолноеИмя = "Отчет.ДетализацияЗаказовНаПроизводство" Тогда
		ТемаПисьма = "Детализация заказов на производство с " + Формат(НачалоПериодаДата, "ДЛФ=Д") + " по " + Формат(КонецПериодаДата, "ДЛФ=Д");
		
	КонецЕсли;
		
	Возврат РаботаСЭлектроннойПочтой.СформироватьЭлектронноеПисьмоСОтчетом(ОтчетТабличныйДокумент, ТемаПисьма, Контрагент, РасширениеВложения);
		
КонецФункции

&НаСервере
Процедура СохранитьПользовательскиеНастройкиНаСервере()
	МодульОтчетов.ДействияПриСохраненииПользовательскихНастроекНаСервере(Отчет,РеквизитФормыВЗначение("Отчет").Метаданные().Имя,Отчет.КомпоновщикНастроек.ПользовательскиеНастройки);
КонецПроцедуры	

//++ №13736
Процедура УстановитьОформлениеПользовательскихНастроек()
	
	// Сводный управленческий отчет производства
	Если НастройкиОтчета.ПолноеИмя = "Отчет.СводныйУправленческийОтчетПроизводства" Тогда
		
		// у параметра "Год" нужно добавить регулирование и задать ширину меньше
		Для Каждого ЭлементПользовательскаяНастройка Из Элементы.КомпоновщикНастроекПользовательскиеНастройки.ПодчиненныеЭлементы Цикл
			
			Для Каждого ПодчиненныйЭлементПользовательскаяНастройка Из ЭлементПользовательскаяНастройка.ПодчиненныеЭлементы Цикл
				
				Если НРег(ПодчиненныйЭлементПользовательскаяНастройка.Заголовок) = НРег("Год") Тогда
					
					ПодчиненныйЭлементПользовательскаяНастройка.КнопкаРегулирования	= Истина;
					ПодчиненныйЭлементПользовательскаяНастройка.АвтоМаксимальнаяШирина = Ложь;
					ПодчиненныйЭлементПользовательскаяНастройка.МаксимальнаяШирина = 5;
		...
		// значение по умолчанию для параметра Год (если не заполнено)
		Для Каждого ЭлементПользовательскойНастройки Из Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
			Если ЭлементПользовательскойНастройки.Параметр = новый ПараметрКомпоновкиДанных("Год") И ЭлементПользовательскойНастройки.Значение = 0 Тогда
				ЭлементПользовательскойНастройки.Значение = Год(ТекущаяДатаСеанса());
			КонецЕсли;
		КонецЦикла;
