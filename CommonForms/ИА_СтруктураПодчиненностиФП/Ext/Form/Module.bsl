...
Процедура ВывестиТабличныйДокумент()
	...
	Макет = ПолучитьОбщийМакет("КСУ_СтруктураПодчиненности");
...
Процедура ВывестиТекущийДокумент(Макет)
	...
	Если Выборка.Следующий() Тогда
		...
		ПереопределяемоеПредставление = ПолучитьПредставлениеДокументаДляПечати(Выборка);
...
Функция ПолучитьПредставлениеДокументаДляПечати(Выборка)
	...
	Если (Выборка.СуммаДокумента <> 0) И (Выборка.СуммаДокумента <> NULL) Тогда
		ПредставлениеДокумента = ПредставлениеДокумента + НСтр("az=' на сумму ';be=' на сумму ';kk=' на сумму ';ky=' на сумму ';md=' на сумму ';ru=' на сумму ';tg=' на сумму ';tk=' на сумму ';uk=' на сумму ';uz=' на сумму '") + Выборка.СуммаДокумента + " " + Выборка.ВалютаДокумента + ".";
	КонецЕсли;
...
Процедура ОбновитьДеревоСтруктурыПодчиненности()
	...
	Иначе
		Сообщить("Документ, для которого сформирован отчет о структуре подчиненности, стал недоступен.");
	КонецЕсли;
...
Функция ПолучитьВыборкуПоРеквизитамДокумента(ДокументСсылка)
	...
	ДополнитьТекстЗапросаПоРеквизитамДокумента(ТекстЗапроса,
	                                           МассивДополнительныхРеквизитовДокумента(МетаданныеДокумента.Имя));
...
Функция ПолучитьСписокДокументовПоКритериюОтбора(ЗначениеКритерияОтбора)
	...
	Если Метаданные.КритерииОтбора.ИА_СвязанныеДокументыФП.Тип.СодержитТип(ТипЗнч(ЗначениеКритерияОтбора))  Тогда
		
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ИА_СвязанныеДокументыФП.Ссылка
		|ИЗ
		|	КритерийОтбора.ИА_СвязанныеДокументыФП(&ЗначениеКритерияОтбора) КАК ИА_СвязанныеДокументыФП";
...
Процедура ВывестиПодчиненныеДокументы(ТекущийДокумент,ДеревоРодитель)
	...
	Для Каждого КлючИЗначение Из КэшПоТипамДокументов Цикл
		...
		|" + ?(КэшРеквизитовДокумента[КлючИЗначение.Ключ]["ВалютаДокумента"], "ВалютаДокумента", "NULL") + " КАК ВалютаДокумента,
		|	#Представление
		|ИЗ
		...
		ДополнитьТекстЗапросаПоРеквизитамДокумента(ТекстПоТипуДокумента,
		                                           МассивДополнительныхРеквизитовДокумента(КлючИЗначение.Ключ));
...
Функция ДобавитьСтрокуВДерево(СтрокиДерева, Выборка)
	...
	ПереопределенноеПредставление = ПолучитьПредставлениеДокументаДляПечати(Выборка);
