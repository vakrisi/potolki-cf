...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ Потолки: №9174 если облако, то в окне подтверждения не показывать реквизиты подключения к консоли сервера
	МассивПеременныхСеанса = ОбщийМодуль.ОпределитьМассивПеременныхСеанса();
	Если МассивПеременныхСеанса.Коробка = 1 Тогда
		
		Элементы.ПортКластераСерверов.Видимость = Ложь;
		Элементы.ГруппаПараметрыУправления.Видимость = Ложь;
