...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.СтруктураСозданныхИОбновленныхЗаказовНаПроизводство <> Неопределено 
		И ТипЗнч(Параметры.СтруктураСозданныхИОбновленныхЗаказовНаПроизводство) = Тип("Структура") Тогда
		
		ЭтотОбъект.Заголовок = "Созданы / обновлены заказы на производство";
		
		Для Каждого ТекущаяСтрока Из Параметры.СтруктураСозданныхИОбновленныхЗаказовНаПроизводство.СозданоНовых Цикл
			НоваяСтрока = СтруктураСозданныхИОбновленныхЗаказовНаПроизводство.Добавить();
			НоваяСтрока.ЗаказНаПроизводство = ТекущаяСтрока;
			НоваяСтрока.Создан = Истина;
		КонецЦикла;
		
		Для Каждого ТекущаяСтрока Из Параметры.СтруктураСозданныхИОбновленныхЗаказовНаПроизводство.Обновлено Цикл
			НоваяСтрока = СтруктураСозданныхИОбновленныхЗаказовНаПроизводство.Добавить();
			НоваяСтрока.ЗаказНаПроизводство = ТекущаяСтрока;
		КонецЦикла;
	...
	Элементы.ГруппаДекорации.Видимость = (СтруктураСозданныхИОбновленныхЗаказовНаПроизводство.Количество() > 0);
	//-- Потолки
...
Процедура ВывестиПредставлениеИКартинку(СтрокаДерева,Макет, ЭтоТекущийОбъект = Ложь, ЭтоПодчиненный = Неопределено)
	...
	ТекущаяОбласть = ТаблицаОтчета.Присоединить(ОбластьОбъект);	
	
	Если СтруктураСозданныхИОбновленныхЗаказовНаПроизводство.Количество() > 0 Тогда		
		
		СтруктураПоиска = Новый Структура("ЗаказНаПроизводство", СтрокаДерева.Ссылка);
		НайденныеСтроки = СтруктураСозданныхИОбновленныхЗаказовНаПроизводство.НайтиСтроки(СтруктураПоиска);
		
		Если НайденныеСтроки.Количество() > 0 Тогда
			
			ЦветОбласти = WebЦвета.ОранжевоКрасный;
			Если НайденныеСтроки[0].Создан Тогда
...
Процедура Провести(Команда)
	...
	ПровестиНаСервере(Элементы.ТаблицаОтчета.ТекущаяОбласть.Расшифровка);
	ОбновитьДеревоСтруктурыПодчиненности();
	Оповестить("ОбновитьАРМПроизводства",,"СтруктураПодчиненности");
...
Процедура ОтменаПроведения(Команда)
	...
	ОтменаПроведенияНаСервере(Элементы.ТаблицаОтчета.ТекущаяОбласть.Расшифровка);
	ОбновитьДеревоСтруктурыПодчиненности();
	Оповестить("ОбновитьАРМПроизводства",,"СтруктураПодчиненности");
...
Процедура ПометкаУдаления(Команда)
	...
	ПометкаУдаленияНаСервере(Элементы.ТаблицаОтчета.ТекущаяОбласть.Расшифровка);
	ОбновитьДеревоСтруктурыПодчиненности();
	Оповестить("ОбновитьАРМПроизводства",,"СтруктураПодчиненности");
...
&НаСервере
Процедура ОтменаПроведенияНаСервере(СсылкаНаДокумент)
...
&НаСервере
Процедура ПометкаУдаленияНаСервере(СсылкаНаДокумент)
...
// необходимо назначить форме
&НаКлиенте
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	
	Если ТипЗнч(ОбъектСсылка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда 
		Если ИмяСобытия = "ЗаписанЗаказНаПроизводство" И Параметр = ОбъектСсылка Тогда
			ВывестиСтруктуруПодчиненности();
		КонецЕсли;
	КонецЕсли;
