...
Процедура ДиаграммаНовыеОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ТекстОтборов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 контрагенты за период: с %2 по %3%4%5%6'"),
		...
		?(ЗначениеЗаполнено(ОтборКатегория), ", с категорией: " + ОтборКатегория, ""),
		?(ЗначениеЗаполнено(ОтборГруппаКонтрагента), ", в группе: " + ОтборГруппаКонтрагента, ""),
...
Процедура ОбновитьДиаграммуНовые()
	...
	Запрос.Текст = 
		...
		|				ТОГДА 0
		|			КОГДА Контрагенты.ДатаСоздания МЕЖДУ &СуществующиеНачало И &СуществующиеКонец
		|				ТОГДА 1
		|			ИНАЧЕ 0
	...
	Если ЗначениеЗаполнено(ОтборКатегория) Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И Контрагенты.Категория = &ОтборКатегория";
		Запрос.УстановитьПараметр("ОтборКатегория", ОтборКатегория);
	КонецЕсли;
	
	Если ОтборВидКонтрагента.Количество() > 0 Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И Контрагенты.ВидКонтрагента В (&ОтборВидКонтрагента)";
		Запрос.УстановитьПараметр("ОтборВидКонтрагента", ОтборВидКонтрагента);
	КонецЕсли;
	
	Если ОтборГруппаКонтрагента.Количество() > 0 Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И Контрагенты.Родитель В ИЕРАРХИИ (&ОтборГруппаКонтрагента)";
		Запрос.УстановитьПараметр("ОтборГруппаКонтрагента", ОтборГруппаКонтрагента);
	КонецЕсли;
	
	Если ОтборМенеджеры.Количество() > 0 Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И Контрагенты.Автор В (&ОтборМенеджеры)";
	...
	Если ЗначениеЗаполнено(ОтборПериодЗначение.ДатаНачала) И ЗначениеЗаполнено(ОтборПериодЗначение.ДатаОкончания) Тогда
		Элементы.ЗаголовокНовые.Заголовок = НСтр("ru = 'Новые контрагенты за период: '") + Формат(ОтборПериодЗначение.ДатаНачала, "ДФ=dd.MM.yy") + " - " + Формат(ОтборПериодЗначение.ДатаОкончания, "ДФ=dd.MM.yy");
	Иначе
		Элементы.ЗаголовокНовые.Заголовок = НСтр("ru = 'Новые контрагенты за: (период не указан)'");
	КонецЕсли;
...
Процедура ОбновитьДиаграммуНовыеПоПериодам()
	...
	Если ЗначениеЗаполнено(ОтборКатегория) Тогда
		ОтборЗапроса.Добавить("Контрагенты.Категория = &ОтборКатегория");
		Запрос.УстановитьПараметр("ОтборКатегория", ОтборКатегория);
	КонецЕсли;
	
	Если ОтборВидКонтрагента.Количество() > 0 Тогда
		ОтборЗапроса.Добавить("Контрагенты.ВидКонтрагента В (&ОтборВидКонтрагента)");
		Запрос.УстановитьПараметр("ОтборВидКонтрагента", ОтборВидКонтрагента);
	КонецЕсли;
	
	Если ОтборГруппаКонтрагента.Количество() > 0 Тогда
		ОтборЗапроса.Добавить("Контрагенты.Родитель В ИЕРАРХИИ (&ОтборГруппаКонтрагента)");
		Запрос.УстановитьПараметр("ОтборГруппаКонтрагента", ОтборГруппаКонтрагента);
	КонецЕсли;
	
	Если ОтборМенеджеры.Количество() > 0 Тогда
		ОтборЗапроса.Добавить("Контрагенты.Автор В (&ОтборМенеджеры)");
		Запрос.УстановитьПараметр("ОтборМенеджеры", ОтборМенеджеры);
	КонецЕсли;
...
Процедура ОбновитьДиаграммуПоМенеджерам()
	...
	Запрос.Текст = 
		...
		|	Контрагенты.Автор КАК Автор,
		|	Контрагенты.Автор.Представление КАК АвторПредставление
		|ИЗ
		|	Справочник.Контрагенты КАК Контрагенты
		|ГДЕ
		|	Контрагенты.ПометкаУдаления = ЛОЖЬ
		...
		|СГРУППИРОВАТЬ ПО
		|	Контрагенты.Автор,
		|	Контрагенты.Автор.Представление
		|
		|УПОРЯДОЧИТЬ ПО
		|	Контрагенты УБЫВ,
		|	Контрагенты.Автор.Наименование";
	...
	Пока ВыборкаОтветственные.Следующий() Цикл
		...
		Если ПорядковыйНомерСерии < МаксимумСерийДиаграммы Тогда
			...
			ПредставлениеМенеджера = ?(ЗначениеЗаполнено(ВыборкаОтветственные.Автор), ВыборкаОтветственные.АвторПредставление, НСтр("ru = '<Менеджер не указан>'"));
			...
			ДиаграммаПоМенеджерам.УстановитьЗначение(Точка, Серия, ВыборкаОтветственные.Контрагенты, ВыборкаОтветственные.Автор, Подсказка);
		...
		Иначе
			...
			Если ТипЗнч(Расшифровка) = Тип("Массив") Тогда
				Расшифровка.Добавить(ВыборкаОтветственные.Автор);
			Иначе
			...
			ТекстПодробно = ?(ЗначениеЗаполнено(ВыборкаОтветственные.Автор), ВыборкаОтветственные.АвторПредставление, НСтр("ru = '<Менеджер не указан>'"))
...
Функция СформироватьРасшифровкуНовых(Признак)
	...
	Макет = ОтчетОбъект.ПолучитьМакет("ПФ_MXL_КонтрагентыРасшифровка");
	...
	ИначеЕсли ВРег(Признак) = ВРег("Существующие") Тогда
		...
		ОтборЗапроса.Добавить("Контрагенты.ДатаСоздания МЕЖДУ &СуществующиеНачало И &СуществующиеКонец");
	КонецЕсли;
...
Функция СформироватьРасшифровкуПоМенеджерам(Менеджеры)
	...
	Запрос.Текст = 
		...
		|	Контрагенты.ПометкаУдаления = ЛОЖЬ
		|	И Контрагенты.ЭтоГруппа = ЛОЖЬ
		|	И Контрагенты.Автор В(&Менеджеры)
		|	И Контрагенты.ДатаСоздания МЕЖДУ &НовыеНачало И &НовыеКонец
		|
		|УПОРЯДОЧИТЬ ПО
		|	Создан УБЫВ,
