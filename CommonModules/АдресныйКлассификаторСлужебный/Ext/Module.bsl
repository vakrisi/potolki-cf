...
Функция РаспознатьАдресЗагруженныеДанные(ЧастиАдреса)
	...
	ТекстЗапроса = 
		...
		|УПОРЯДОЧИТЬ ПО
		|	Уровень
		//++ Потолки: административное деление в приоритете
		|	, Муниципальный
		//-- Потолки
		|";
...
Функция КаскадныйСписокАвтоподбораНаселенногоПункта()
	...
	ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 10
				   ...
				   //++ Потолки: пока не используем муниципальное деление
				   |	И ЛОЖЬ
				   //-- Потолки
	               |
	               |УПОРЯДОЧИТЬ ПО
...
Процедура ЗаполнитьАдресаДляИнтерактивногоВыбораВнутр(Результат, Родитель, Уровень, ТипАдреса, ДополнительныеПараметры)
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   //++ Потолки
				   |	, АдресныеОбъекты.РодительскийИдентификатор КАК ИдентификаторРодителя
				   |	, ЕСТЬNULL(РодительАО.Наименование, """") + "" "" + ЕСТЬNULL(РодительАО.Сокращение, """") КАК ПредставлениеРодителя
				   //-- Потолки
	               ...
	               |		ПО (АдресныеОбъекты.КодСубъектаРФ = ЗагруженныеВерсииАдресныхСведений.КодСубъектаРФ)
				   //++ Потолки
				   |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК РодительАО
	               |		ПО (АдресныеОбъекты.РодительскийИдентификатор = РодительАО.Идентификатор)
				   //-- Потолки
	               |ГДЕ
	...
	Если Уровень > 1 Тогда
		...
		Если НЕ Уровень = 7 Тогда
			Запрос.УстановитьПараметр("Идентификатор", Родитель);
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборПоИдентификатору", "И АдресныеОбъекты.РодительскийИдентификатор = &Идентификатор");
		Иначе
			// ищем все улицы в подчинении
			МассивИД = Новый Массив;
			МассивИД.Добавить(Родитель);
			...
			|	РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
			|ГДЕ
			|	НЕ АдресныеОбъекты.Уровень = &Уровень
			|	И АдресныеОбъекты.РодительскийИдентификатор В (&Идентификатор)";
			Если ЭтоМуниципальныйАдрес(ТипАдреса) Тогда
				ЗапросИД.Текст = СтрЗаменить(ЗапросИД.Текст, "РодительскийИдентификатор", "МуниципальныйРодительскийИдентификатор");
			КонецЕсли;
			
			Для Индекс = 1 По 5 Цикл
				ЗапросИД.УстановитьПараметр("Уровень", Уровень);
				ЗапросИД.УстановитьПараметр("Идентификатор", МассивИД);
				МассивНовыхИД = ЗапросИД.Выполнить().Выгрузить().ВыгрузитьКолонку("Идентификатор");
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивИД, МассивНовыхИД, Истина);
			КонецЦикла;
			
			Запрос.УстановитьПараметр("Идентификатор", МассивИД);
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборПоИдентификатору", "И АдресныеОбъекты.РодительскийИдентификатор В (&Идентификатор)");
		КонецЕсли;
		//-- Потолки
	Иначе
	...
	Для Каждого Строка Из РезультатЗапроса.Выгрузить() Цикл
		...
		//++ Потолки #17014
		Если Уровень = 7 
			И ЗначениеЗаполнено(СокрЛП(Строка.ПредставлениеРодителя))
			// если не в прямом подчинении
			И НЕ Строка.ИдентификаторРодителя = Родитель Тогда
			//Строка.Представление = Строка.Представление + " (" + Строка.ПредставлениеРодителя + ")";
			Строка.Представление = Строка.ПредставлениеРодителя + ", " + Строка.Представление;
		КонецЕсли;
		//-- Потолки		
...
Процедура ДополнитьТекстЗапроса(ТекстЗапроса)
	...
	ТекстЗапроса = "
		...
		|	СвязиАдресныхОбъектов.МуниципальныйРодительскийИдентификатор В (&Идентификатор)
		//-- Потолки
		|	&ОтборПоСтроке
		|
		|ОБЪЕДИНИТЬ ВСЕ
...
Функция ДоступныеВерсииАдресныхСведений(ПараметрыВыгрузки = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	//++ Потолки
	//Если НЕ ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
	Если НЕ ДоступнаЗагрузкаАдресныхСведенийИзИнтернет() Тогда
	//-- Потолки
		Возврат Результат;
	КонецЕсли;
...
Функция JSONВКонтактнуюИнформациюПоПолям(Значение) Экспорт
