...
Функция ЭтоПолноправныйПользователь(Пользователь = Неопределено,
	...
	Если ПроверитьРольПолныеПрава
	   ...
	   И НЕ РольДоступна(Метаданные.Роли.АдминистраторСистемы) Тогда // Не заменять на РолиДоступны.
	   //--
		Возврат Ложь;
	КонецЕсли;
...
// Создает пользователя ИБ с указанным именем с правами администратора,
// ВНИМАНИЕ! Имя пользователя не должно содержать пробелы.
//
// Параметры:
//  ИмяПользователя       - строка с именем создаваемого пользователя
//
// Возвращаемое значение:
//  Неопределено                  - при ошибке создания пользователя.
//  СправочникСсылка.Пользователи - пользователь в справочнике, с которым сопоставлен созданный пользователь
//
Функция СоздатьПользователя(ИмяПользователя, ПоказыватьВСпискеВыбора = Истина) Экспорт
