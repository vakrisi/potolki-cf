...
Процедура ПриСозданииНаСервере_ФормаСписка(Форма, Список, Расположение = Неопределено) Экспорт
	...
	//+- Потолки для РМ, где несколько списков
	//РеквизитСписокСостояние = Форма.Элементы.Вставить("СостояниеОригиналПолучен",Тип("ПолеФормы"),Список,Расположение);
	РеквизитСписокСостояние = Форма.Элементы.Вставить(Список.Имя + "СостояниеОригиналПолучен",Тип("ПолеФормы"),Список,Расположение);
	...
	РеквизитСписокСостояние = Форма.Элементы.Вставить(Список.Имя + "СостояниеОригиналаПервичногоДокумента",Тип("ПолеФормы"),Список,Расположение);
	...
	// Создание списка
	Реквизиты = Новый Массив;
	Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "СписокВыбораСостоянийОригинала") Тогда //+- Потолки
		Реквизиты.Добавить(Новый РеквизитФормы("СписокВыбораСостоянийОригинала", Новый ОписаниеТипов("СписокЗначений")));	
		Форма.ИзменитьРеквизиты(Реквизиты);
	КонецЕсли;
