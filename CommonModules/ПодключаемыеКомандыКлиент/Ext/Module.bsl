...
Процедура ПродолжитьВыполнениеКоманды(Контекст)
	...
	Если Контекст.ТребуетсяЗапись Тогда
		...
		Если Источник.Ссылка.Пустая()
			...
			ПродолжитьВыполнениеКомандыПослеПодтвержденияЗаписи(КодВозвратаДиалога.ОК, Контекст);//nva
			//nva---
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	//++ Потолки
	Если Контекст.ТребуетсяПроведение 
		И ОписаниеКоманды.Вид = "Печать" 
		И ОбщийМодульВызовСервера.РазрешеноПечататьНепроведенныеДокументы() Тогда
	...
	Если Контекст.ТребуетсяПроведение Тогда
		...
		Если ИнформацияОДокументах.Непроведенные.Количество() > 0 Тогда
			Если ИнформацияОДокументах.ДоступноПравоПроведения Тогда
				...
				//Кнопки.Добавить(КодВозвратаДиалога.Да); //, НСтр("ru = 'Продолжить'")); //#13969
				//Кнопки.Добавить(КодВозвратаДиалога.Отмена);
				//ПоказатьВопрос(Обработчик, ТекстВопроса, Кнопки);
				ПродолжитьВыполнениеКомандыПослеПодтверждениеПроведения(КодВозвратаДиалога.Да, Контекст);//nva
				//nva---
			Иначе
				Если ИнформацияОДокументах.Непроведенные.Количество() = 1 Тогда
