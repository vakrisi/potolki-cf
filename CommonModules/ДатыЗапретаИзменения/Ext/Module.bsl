...
//  РежимПроведения - Булево - параметр, передаваемый в подписку на событие ПередЗаписью.
//
Процедура ПроверитьДатуЗапретаБудущимЧисломПередЗаписьюДокумента(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Отказ = КонецДня(Источник.Дата) > КонецДня(ТекущаяДатаСеанса()) И РежимЗаписи = РежимЗаписиДокумента.Проведение И Константы.ЗапретПроведенияДокументовБудущимЧислом.Получить();
	Если Отказ Тогда
		ОписаниеОшибки = "Настройками программы запрещено проведение документов будущим числом!";
