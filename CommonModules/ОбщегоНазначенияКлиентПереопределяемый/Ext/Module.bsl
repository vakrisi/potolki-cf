...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПредупреждениеПроверкаКлиентаОбработкаЗавершения", ЭтотОбъект);
	ПоказатьПредупреждение(ОписаниеОповещения, НСтр("ru = 'Работа программы в режиме толстого клиента невозможна.'"));
	Возврат;
	#КонецЕсли
	...
	//ОписаниеОповещения = Новый ОписаниеОповещения("ПредупреждениеПроверкаКлиентаОбработкаЗавершения", ЭтотОбъект);
	//ПоказатьПредупреждение(ОписаниеОповещения, НСтр("ru = 'Работа программы в режиме веб-клиента невозможна.'"));
	//Возврат;
	//#КонецЕсли

	//Если НЕ НРег(ИмяПользователя()) = НРег("УстановкаОбновлений") Тогда  // #7604 закомментировано
		ОбщийМодуль.ЗапуститьФоновоеЗаданиеПолучитьУведомленияОНовыхВерсиях();		
		ОткрытьФорму("ОбщаяФорма.УведомлениеОВыходеНовыхВерсий");
		
		//ОткрытьФорму("ОбщаяФорма.УведомлениеОРезервномКопировании");
	//КонецЕсли;
	
	//Проверка на сетевой путь и наличия запрещенных символов в пути
	Если СтрНайти(ВРег(СтрокаСоединенияИнформационнойБазы()), "FILE=""\\") > 0 Тогда
		Если СтрНайти(СтрокаСоединенияИнформационнойБазы(), "&") > 0
			ИЛИ СтрНайти(СтрокаСоединенияИнформационнойБазы(), "#") > 0	
		Тогда
			Сообщить("В названии пути подключения к базе имеются запрещенные символы ""&"" или ""#"", для корректной работы системы, необходимо переименовать путь на общем сетевом ресурсе!");
		КонецЕсли;
	КонецЕсли;
	
	//Если НЕ НРег(ИмяПользователя()) = НРег("УстановкаОбновлений") Тогда // #7604 закомментировано

		СтруктураПоУведомлениям = ОбщийМодульВызовСервера.ПолучитьСтруктуруУведомлений();
		Если СтруктураПоУведомлениям.ТребуетсяУведомление Тогда
			ОткрытьФорму("ОбщаяФорма.ЦентрУведомлений", СтруктураПоУведомлениям);
		КонецЕсли;
	//КонецЕсли;
	
	ОбщийМодульВызовСервера.НастроитьРолиПользователя();
	
	//Если ОбщийМодульВызовСервера.ВерсияКонфигурациии() = "3.0.100.0" Тогда
	//	ОткрытьФорму("Обработка.СервисныеОбработки.Форма.Перед31");	
	//КонецЕсли;
...
//++ Потолки
Процедура ПредупреждениеПроверкаКлиентаОбработкаЗавершения(Неопределен) Экспорт
