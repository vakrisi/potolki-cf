...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	//++ Потолки
	Если ОбщийМодуль.ЛицензионноеСоглашениеНеПринято() и НЕ НРег(ИмяПользователя()) = НРег("Администратор") 
		//и НЕ НРег(ИмяПользователя()) = НРег("УстановкаОбновлений")  // #7604 закомментировано
		и НЕ НРег(ИмяПользователя()) = НРег("support") //++ №8209 пользователя Администратор переименован
	Тогда
		ОткрытьФорму("ОбщаяФорма.ЛицензионноеСоглашение");
	КонецЕсли;
	
	Если ОбщийМодуль.НужноЗапуститьМастерПервоначальнойНастройки() Тогда
		ОткрытьФорму("Обработка.МастерПервоначальнойНастройки.Форма.Форма");
	КонецЕсли;
	//-- Потолки
