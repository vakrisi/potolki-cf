...
Процедура ОпределитьИсключаемыеОтчеты(ИсключаемыеОтчеты) Экспорт
	...
	ИсключаемыеОтчеты.Добавить(Метаданные.Отчеты.ГрафикЗамеров);
	ИсключаемыеОтчеты.Добавить(Метаданные.Отчеты.ГрафикМонтажей);
	ИсключаемыеОтчеты.Добавить(Метаданные.Отчеты.ПрайсЛист);
	ИсключаемыеОтчеты.Добавить(Метаданные.Отчеты.СравнительныйАнализРаботыМенеджеров);
	ИсключаемыеОтчеты.Добавить(Метаданные.Отчеты.СравнениеЗаказДоговораСМонтажами);
	
	Для каждого Отчет из Метаданные.Отчеты Цикл
		Если СтрНайти(Отчет.Имя, "УДАЛИТЬ_") > 0 Тогда
			ИсключаемыеОтчеты.Добавить(Отчет);	
		КонецЕсли;
	КонецЦикла;	
