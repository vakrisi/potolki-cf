...
Функция ДатаЗапретаПоОписанию(ОписаниеДатыЗапрета, ДатаЗапрета, НачалоДня = '00010101') Экспорт
	...
	//++ Потолки (#9910)
	ИначеЕсли ВариантДатыЗапрета = "СтаршеУказанныхДней" ИЛИ ВариантДатыЗапрета = "СтаршеУказанныхМесяцев" Тогда
		ТекущаяДатаЗапрета    = '00010101'; 
		ПредыдущаяДатаЗапрета = '00010101';
		ОбщийМодульКлиентСервер.РасчетДатыЗапретаРасширение(ВариантДатыЗапрета, НачалоДня, КоличествоДнейРазрешения, КоличествоДнейРазрешения, ТекущаяДатаЗапрета, ПредыдущаяДатаЗапрета);
		КоличествоДнейРазрешения = 0;
	//-- Потолки (#9910)
