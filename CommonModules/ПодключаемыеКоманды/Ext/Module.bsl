...
Процедура ВывестиКоманды(Форма, Команды, ПараметрыРазмещения)
	...
	Для Каждого ВидКоманд Из ВидыКоманд Цикл
		...
		Для Каждого Команда Из КомандыВида Цикл
			...
			//++ Потолки
			Если Команда.Вид = "ОбработкаОбновления" И ПараметрыРазмещения.ЭтоФормаОбъекта Тогда
				Если НЕ ОбщийМодульВызовСервера.ТребуетсяВыводКомандыПодключеннойКОбъекту(Форма.Объект.Ссылка, Команда) Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			//-- Потолки
			...
			//++ Потолки
			ПодключаемыеКомандыПереопределяемый.ПриВыводеКоманды(Команда, КомандаФормы, КнопкаФормы);
			//--
