...
Процедура Подключаемый_УстановитьСостояниеОригинала(Ссылка, Параметры) Экспорт
	...
	//++ Потолки
	Если Не УчетОригиналовПервичныхДокументовПотолкиВызовСервера.ПраваНаИзменениеСостояния(Ссылка) Тогда
...
Процедура УстановитьСостояниеОригинала(Команда, Форма, Список) Экспорт
	...
	//++ Потолки
	Если Не УчетОригиналовПервичныхДокументовПотолкиВызовСервера.ПраваНаИзменениеСостояния(Список.ТекущиеДанные.Ссылка) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'У пользователя недостаточно прав на изменение состояния оригинала первичного документа'"));
		Возврат;
	КонецЕсли;
	//-- Потолки
...
Процедура ОткрытьМенюВыбораСостояния(Форма, Источник) Экспорт 
	...
	Если ТипЗнч(Источник) = Тип("ТаблицаФормы") Тогда
		ОбъектСсылка = Источник.ТекущиеДанные.Ссылка;		
	Иначе
		ОбъектСсылка = Форма.Объект.Ссылка;
	КонецЕсли;
	Если Не УчетОригиналовПервичныхДокументовПотолкиВызовСервера.ПраваНаИзменениеСостояния(ОбъектСсылка) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'У пользователя недостаточно прав на изменение состояния оригинала первичного документа'"));
		Возврат;
	КонецЕсли;
	//-- Потолки
	...
	Если ТипЗнч(Источник) = Тип("ТаблицаФормы") Тогда
		...
		ЭлементСостояниеОригиналаПервичногоДокумента = Форма.Элементы[Источник.Имя + "СостояниеОригиналаПервичногоДокумента"]; //+- Потолки 
		...
		Иначе
			...
			Если ДанныеЗаписи.ОбщееСостояние Или Не ЗначениеЗаполнено(ДанныеЗаписи.СостояниеОригиналаПервичногоДокумента) Тогда
				...
				КонецЕсли;
				Форма.ПоказатьВыборИзМеню(ОписаниеОповещения,Форма.СписокВыбораСостоянийОригинала,ЭлементСостояниеОригиналаПервичногоДокумента); //Форма.Элементы.СостояниеОригиналаПервичногоДокумента);
			Иначе
				УточнитьПоПечатнымФормам = Форма.СписокВыбораСостоянийОригинала.НайтиПоЗначению("Уточнитьпопечатнымформам");
				Если УточнитьПоПечатнымФормам <> Неопределено Тогда
...
Процедура СписокВыбор(ИмяПоля, Форма, Список, СтандартнаяОбработка) Экспорт 
	...
	Если СтрНайти(ИмяПоля, "СостояниеОригиналаПервичногоДокумента") Или СтрНайти(ИмяПоля, "СостояниеОригиналПолучен") Тогда
		...
		Если УчетОригиналовПервичныхДокументовВызовСервера.ЭтоОбъектУчета(Список.ТекущиеДанные.Ссылка) Тогда
			...
			Если СтрНайти(ИмяПоля, "СостояниеОригиналаПервичногоДокумента") Тогда
				ОткрытьМенюВыбораСостояния(Форма, Список);
			//ИначеЕсли ИмяПоля = "СостояниеОригиналПолучен" Тогда
			ИначеЕсли СтрНайти(ИмяПоля, "СостояниеОригиналПолучен") Тогда
