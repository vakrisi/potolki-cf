...
Функция ДеревоРолей(ПоПодсистемам = Истина, Назначение = "ДляПользователей") Экспорт
	...
	//++ Потолки #13730 поднимаем подсистему "Рабочие места" и "Отчеты" выше
	Если ПоПодсистемам Тогда
		Дерево.Строки.Сдвинуть(Дерево.Строки[Дерево.Строки.Количество()-1], -3);
		
		Дерево.Строки.Сдвинуть(Дерево.Строки[Дерево.Строки.Количество()-1], -1);
	КонецЕсли;
	//-- Потолки
...
Процедура ЗаполнитьПодсистемыИРоли(КоллекцияСтрокДерева, Подсистемы, НедоступныеРоли, ВсеРоли = Неопределено)
	...
	//++ #11364
	ПодсистемыПрав = Метаданные.Подсистемы.СлужебныеПодсистемы.Подсистемы.Права.Подсистемы;
	
	Если Подсистемы = Неопределено Тогда
		Подсистемы = Метаданные.Подсистемы;
		//++ #11364
		Подсистемы = ПодсистемыПрав;
	КонецЕсли;
	...
	Для Каждого Подсистема Из Подсистемы Цикл
		...
		//++ #11364
		Если НЕ Подсистема = ПодсистемыПрав.БазовыеПрава 
			И НЕ Подсистема = ПодсистемыПрав.ДополнительныеПрава 
			И НЕ Подсистема = ПодсистемыПрав.РабочиеМеста // #13730
			И НЕ Подсистема = ПодсистемыПрав.Документы    // #14749
			И НЕ Подсистема = ПодсистемыПрав.Отчеты       // #14749
			Тогда
			Продолжить;
		КонецЕсли;
		//--
