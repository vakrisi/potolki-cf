...
Процедура ПриИзменении(Форма, Элемент) Экспорт
	...
	ПриИзмененииКонтактнойИнформации(Форма, Элемент, Истина);
...
Процедура ДобавитьКонтактнуюИнформацию(Форма, ИмяКоманды) Экспорт
	
	ИмяЭлементаДляРазмещения = "ГруппаДополнительнаяКонтактнаяИнформация";
	
	Оповещение = Новый ОписаниеОповещения("ДополнительнаяКонтактнаяИнформацияДобавитьПолеВводаЗавершение", ЭтотОбъект, Новый Структура);
	Оповещение.ДополнительныеПараметры.Вставить("Форма", Форма);
	...
	Форма.ПоказатьВыборИзМеню(Оповещение, Форма.ПараметрыДополнительнойКонтактнойИнформации[ИмяЭлементаДляРазмещения].СписокДобавляемыхЭлементов, Форма.Элементы[ИмяКоманды]);
...
Процедура ДополнительнаяКонтактнаяИнформацияДобавитьПолеВводаЗавершение(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Результат.Вставить("ИдентификаторСтрокиТабличнойЧасти", Новый УникальныйИдентификатор);
	...
	//	Результат.Вставить("ОбновитьКонтекстноеМеню", Истина);
	//КонецЕсли;
	Если ВыбранныйЭлемент.Значение.Тип = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон") Тогда
		Результат.Вставить("ЗаполнитьКодСтраны", Истина);
	КонецЕсли;
...
Функция ПолучитьСтрокуДополнительныхЗначений(Форма, Элемент)
	...
	Строки = Форма.ДополнительнаяКонтактнаяИнформацияОписаниеРеквизитов.НайтиСтроки(Отбор);
...
Функция ПолучитьТелефонИзКонтактнойИнформацииФормы(Форма, ТолькоПредставление = Истина) Экспорт
	
	Результат = Новый Массив;
	
	ОтборПоВиду = Новый Структура("Вид", ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ТелефонКонтрагента"));
...
Процедура ОбновитьКонтактнуюИнформациюФормы(Форма, Результат)
	...
	Форма.Подключаемый_ОбновитьДополнительнуюКонтактнуюИнформацию(Результат);
	
КонецПроцедуры

Процедура ПриИзмененииКонтактнойИнформации(Форма, Элемент, ОбновитьФорму)
