...
Процедура ПриОпределенииНастроек(Настройки) Экспорт
	...
	// ПОТОЛКИ
	Реквизиты = Новый Соответствие;
	Реквизиты.Вставить("СуммаДокумента", "Сумма");

	Настройки.Реквизиты.Вставить(Метаданные.Документы.ПриходныйКассовыйОрдер.ПолноеИмя(), Реквизиты);
	Настройки.Реквизиты.Вставить(Метаданные.Документы.РасходныйКассовыйОрдер.ПолноеИмя(), Реквизиты);
	Настройки.Реквизиты.Вставить(Метаданные.Документы.ОплатаБанковскойКартой.ПолноеИмя(), Реквизиты);
	Настройки.Реквизиты.Вставить(Метаданные.Документы.ПоступлениеНаРасчетныйСчет.ПолноеИмя(), Реквизиты);
	Настройки.Реквизиты.Вставить(Метаданные.Документы.СписаниеСРасчетногоСчета.ПолноеИмя(), Реквизиты);
...
Процедура ПриПолученииПредставления(ТипДанных, Данные, Представление, СтандартнаяОбработка) Экспорт
	...
	Если (Данные.СуммаДокумента <> 0) И (Данные.СуммаДокумента <> NULL) Тогда
		Представление = Представление + ?(ПараметрыСеанса.СкрыватьСуммуДокументовВСтруктуреПодчиненности,"", " " + НСтр("ru='на сумму'") + " " + Данные.СуммаДокумента + " " + Данные.Валюта);	
	КонецЕсли;
	
	Если ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.Монтаж") Тогда
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Данные.Ссылка, "ВидОперации, Результат, ДатаМонтажа, ВремяНачала, ВремяОкончания");
		Если НЕ РеквизитыДокумента.ВидОперации = Справочники.ВидыМонтажа.Продажа Тогда
			Представление = СтрШаблон("%1 ТЕКУЩИЙ СТАТУС: %2. ДАТА ВЫПОЛНЕНИЯ: %3 %4-%5", Представление, РеквизитыДокумента.Результат, Формат(РеквизитыДокумента.ДатаМонтажа, "ДЛФ=Д"), Формат(РеквизитыДокумента.ВремяНачала, "ДЛФ=В"), Формат(РеквизитыДокумента.ВремяОкончания, "ДЛФ=В") );	
		КонецЕсли;
	КонецЕсли;
	
	Если ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Данные.Ссылка, "Контрагент");
		Представление = СтрШаблон("%1 ПОСТАВЩИК: %2", Представление, РеквизитыДокумента.Контрагент);
	КонецЕсли;
	
	Если ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Данные.Ссылка, "ТекущийСтатус");
		Представление = СтрШаблон("%1 ТЕКУЩИЙ СТАТУС: %2", Представление, РеквизитыДокумента.ТекущийСтатус);
