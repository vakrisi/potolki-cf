...
Функция ОписанияДатЗапрета() Экспорт
	...
	//++ Потолки (#9910)
	Список.Вставить("СтаршеУказанныхДней",   НСтр("ru = 'Старше указанного количества дней'"));
	Список.Вставить("СтаршеУказанныхМесяцев",НСтр("ru = 'Старше указанного количества месяцев'"));
	//-- Потолки (#9910)
	//++ Потолки (#16658)
	Список.Вставить("БезЗапрета",			 НСтр("ru = 'Без запрета'"));
	//++ Потолки (#16658)
	Возврат Список;
...
Процедура УточнитьНастройкуДатыЗапретаПриИзменении(Контекст, РассчитатьДатуЗапрета = Истина) Экспорт
	...
	//++ Потолки (#16658)
	Если Контекст.ДатаЗапрета = '00010101' И Контекст.ОписаниеДатыЗапрета = "ПроизвольнаяДата" Тогда
		Контекст.ОписаниеДатыЗапрета = "БезЗапрета";
	КонецЕсли;
	//-- Потолки (#16658)
	//++ Потолки (#9910)
	...
	//++ Потолки (#16658)
	Если Контекст.ОписаниеДатыЗапрета = "БезЗапрета" Тогда
		Контекст.ДатаЗапрета = "00010101";
		Контекст.КоличествоДнейРазрешения = 0;
		Возврат;
	КонецЕсли;
	//++ Потолки (#16658)
	//++ Потолки (#9910)
	ОбщийМодульКлиентСервер.ОбщаяДатаЗапретаСОписаниемПриИзмененииРасширение(Контекст);
	//-- Потолки (#9910)
	
	РасчетныеДатыЗапрета = РасчетДатыЗапрета(
		Контекст.ОписаниеДатыЗапрета, Контекст.НачалоДня
		//++ Потолки (#9910)
		, Контекст.СтаршеУказанныхДней, Контекст.СтаршеУказанныхМесяцев);
		//-- Потолки (#9910)
...
Функция РасчетДатыЗапрета(Знач ВариантДатыЗапрета, Знач ТекущаяДатаНаСервере
	...
	//++ Потолки (#9910)
	ОбщийМодульКлиентСервер.РасчетДатыЗапретаРасширение(ВариантДатыЗапрета, ТекущаяДатаНаСервере, СтаршеУказанныхДней, СтаршеУказанныхМесяцев, ТекущаяДатаЗапрета, ПредыдущаяДатаЗапрета);
	//-- Потолки (#9910)
